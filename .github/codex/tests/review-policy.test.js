...
test("redispatches the trusted workflow from the development default branch", () => {
  const redispatchRefs = [...WORKFLOW.matchAll(/ref:\s*"([^"]+)"/g)].map(
    (match) => match[1],
  );

  assert.deepEqual(redispatchRefs, ["dev", "dev"]);
});
