...
2026-09-06 旧测试归档后：world 114 项通过、1 项失败、2 项真实探测跳过；domain 429 项通过。本轮恢复的 89 项离线测试全部通过。日记 Fake 更新为当前 `ensure_dependencies()`，随机抽样改为受控结果；动态数据库 fixture 恢复环境变量并关闭数据库。迁移前后测试函数清单一致，无遗漏或重复副本。通过的场景是既有实现的回归证据，无人为制造的 RED。
