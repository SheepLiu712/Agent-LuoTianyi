...
def test_dynamic_visibility_and_comment_isolation(db_manager: DatabaseManager):
    ...
    ok, _, private_dynamic = db_manager.create_dynamic(
        author_type="user",
    ...
    ok, _, public_dynamic = db_manager.create_dynamic(
        author_type="agent",
        author_id="luotianyi",
        owner_user_id=None,
        visibility="global",
    ...
    ok, _, user1_comments = db_manager.list_dynamic_comments_for_user(
        user1["user_uuid"],
        public_dynamic["id"],
    )
    ok2, _, user2_comments = db_manager.list_dynamic_comments_for_user(
        user2["user_uuid"],
        public_dynamic["id"],
    )
...
def test_system_dynamic_skips_memory_and_reply_and_disallows_comments(db_manager: DatabaseManager):
    ...
    ok, _, dynamic = dynamic_capability.publish_system_dynamic(
    ...
    pending_memory = db_manager.list_pending_dynamic_posts_for_memory()
...
def test_admin_dynamic_time_filters(db_manager: DatabaseManager):
    ...
    feed = db_manager.admin_list_dynamics(created_after=recent_cutoff)
    assert feed["items"] == []

    older_cutoff = datetime.now() - timedelta(days=5)
    feed = db_manager.admin_list_dynamics(created_after=older_cutoff, created_before=datetime.now() - timedelta(days=1))
    assert [item["id"] for item in feed["items"]] == [dynamic["id"]]

    comments = db_manager.admin_list_dynamic_comments(
        dynamic["id"],
...
def test_admin_dynamic_list_includes_comment_count(db_manager: DatabaseManager):
    ...
    feed = db_manager.admin_list_dynamics(owner_user_id=auth["user_uuid"])
...
def test_citywalk_task_publishes_global_dynamic(db_manager: DatabaseManager, tmp_path: Path):
    ...
    assert feed["items"][0]["content"] == "今天在上海的武康路散步，风很舒服。"
...
def test_learn_song_task_publishes_global_dynamic(db_manager: DatabaseManager):
    ...
    assert feed["items"][0]["content"] == "今天学会了《告死鸟》，下次可以唱给你听。"
...
def test_dynamic_interaction_task_replies_and_updates_status(db_manager: DatabaseManager):
    ...
    ok, _, created = db_manager.create_dynamic(
        author_type="user",
        author_id=auth["user_uuid"],
        owner_user_id=auth["user_uuid"],
        visibility="private",
    ...
    class FakeReplyLLM:
        async def generate_response(self, **kwargs):
            return '{"should_reply": true, "reply": "我看到你很努力地撑过来了，辛苦啦。"}'
    ...
    ok, _, comments = db_manager.list_dynamic_comments_for_user(auth["user_uuid"], dynamic_id)
    assert ok is True
