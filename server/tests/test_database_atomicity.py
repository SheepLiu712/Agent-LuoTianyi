...
def test_concurrent_registration_claims_invite_once(db_manager, monkeypatch):
    ...
    monkeypatch.setattr(credential_service, "_hash_password", synchronized_hash)
    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(
            executor.map(
                lambda username: db_manager.credential_service.register_user(username, "password", "ONE-TIME-CODE"),
                ("alice", "bob"),
            )
        )
...
def test_registration_commit_failure_rolls_back_user_and_invite(db_manager, monkeypatch):
    ...
    ok, _ = db_manager.credential_service.register_user("rollback-register", "password", "ROLLBACK-CODE")
...
def test_reset_invalidates_old_and_new_username_cache_keys(db_manager):
    ...
    assert db_manager.credential_service.register_user("old-name", "password", "RESET-CODE")[0] is True
    original_uuid = db_manager.credential_service.get_user_uuid_by_username("old-name")

    assert db_manager.credential_service.reset_account("RESET-CODE", "new-name", "new-password")[0] is True
    add_invite(db_manager, "REUSE-CODE")
    assert db_manager.credential_service.register_user("old-name", "other-password", "REUSE-CODE")[0] is True

    reused_uuid = db_manager.credential_service.get_user_uuid_by_username("old-name")
    assert reused_uuid != original_uuid
    assert db_manager.credential_service.get_user_uuid_by_username("new-name") == original_uuid
...
def test_cache_errors_after_commit_do_not_report_false_failure(db_manager, monkeypatch):
    ...
    assert db_manager.credential_service.register_user("cache-user", "password", "CACHE-FAIL-CODE")[0] is True

    monkeypatch.setattr(redis, "delete", fail_cache_operation)
    assert db_manager.credential_service.reset_account("CACHE-FAIL-CODE", "cache-user-new", "password")[0] is True
