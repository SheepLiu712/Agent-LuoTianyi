"""Speaking Skill 私有 TTS 实现的真实集成测试。"""
...
from src.agent.skills.expression.speaking import SpeechBackend
...
def capability_config():
    return load_config("config/config.json")["agent_runtime"]["skills"]["speaking"]


@pytest.fixture(scope="module")
def speech_capability(capability_config):
    speech = SpeechBackend(capability_config["characters"])
    try:
        yield speech
    finally:
...
def test_tts_config_is_valid(capability_config):
    tts_cfg = capability_config.get("characters", {})
    ...
    for key in required_paths:
        ...
        assert value, f"agent_runtime.skills.speaking.characters.{CHARACTER_ID}.{key} is required"
