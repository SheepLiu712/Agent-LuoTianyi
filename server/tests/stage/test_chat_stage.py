...
def report(request, *, consumed=(), plans=()):
    ...
    if isinstance(request.stimulus, (d.TextMessage, d.ImageMessage, d.VoiceMessage)):
        prepared = d.PreprocessedInput(stimulus_id=request.stimulus.stimulus_id,
    ...
    return d.HandlingReport(request_id=request.request_id, trigger_stimulus_id=request.stimulus.stimulus_id,
        ...
        preprocessed_input=prepared, emitted_plan_ids=tuple(plans), error_code=None, retryable=False)
...
class RecordingAgent:
    ...
    async def handle_stimulus(self, request, sink, *, context=None):
        ...
        return report(request, consumed=tuple(s.stimulus_id for s in request.interaction.pending_stimuli)
                      if isinstance(request.stimulus, d.InteractionDeadline) else ())
...
class StageContextFactory:
    ...
    async def create(self, interaction_id, *, user_id):
        context = SimpleNamespace(identity=SimpleNamespace(interaction_id=interaction_id,
            user_id=user_id, character_id=self.character_id), closed=False)
        async def close():
            context.closed = True
...
async def setup(agent=None, config=None):
    ...
    stage = await ChatStage.create(user_id="user", character_id="luotianyi", agent=agent, adapter=adapter,
                                   config={"response_wait": 0.02, **(config or {})}, context_factory=StageContextFactory())
...
async def test_manager_reconnect_old_disconnect_and_offline_expiry():
    ...
    manager = StageManager(get_context_factory=StageContextFactory, get_agent=lambda _: agent, adapter=adapter, config={"offline_timeout": 0.15})
...
async def test_reconnect_during_ending_creates_new_interaction():
    ...
    async def handle(request, sink):
        ...
        return report(request)
    adapter = WebSocketAdapter()
    manager = StageManager(get_context_factory=StageContextFactory, get_agent=lambda _: RecordingAgent(handle), adapter=adapter, config={"offline_timeout": 0})
    old = WebSocketConnection(Socket(), "user", "用户")
    stage = await manager.connect(old, "luotianyi")
    await manager.disconnect(old)
...
async def test_stage_releases_context_after_ending_handler():
    ...
    agent = Agent(character_id="luotianyi", stimulus_router=StimulusRouter([
        (d.StimulusKind.INTERACTION_ENDING, Ending())]))
    stage = await ChatStage.create(user_id="user", character_id="luotianyi", agent=agent,
                                   adapter=adapter, context_factory=factory)
...
async def test_real_adapter_stage_agent_prepared_say_chain(tmp_path):
    ...
    class TextHandler:
        async def handle(self, request, plans):
            ...
            return report(request, consumed=tuple(s.stimulus_id for s in request.interaction.pending_stimuli), plans=(accepted.plan_id,))
    agent = Agent(character_id="luotianyi", stimulus_router=StimulusRouter([
...
async def test_stage_construction_failure_closes_created_context():
    factory = StageContextFactory()
    with pytest.raises(ValueError):
        await ChatStage.create(user_id="user", character_id="luotianyi", agent=RecordingAgent(),
            adapter=WebSocketAdapter(), context_factory=factory, config={"max_stimuli": 0})
