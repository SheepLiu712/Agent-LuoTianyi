...
def test_dynamic_operations_accept_nested_module_config_and_degrade_on_invalid_llm():
    ...
    bad_capability = DynamicOperations(
        {
            "dynamic_composer": {
                "llm_module": {
                    "llm": {"name": ""},
...
def test_dynamic_replier_passes_thread_comments_to_llm():
    ...
    dynamic_operations.replier._reply_llm = FakeLLM()
    item = {
        "author_type": "user",
        "author_name": "Dpon",
        "username": "Dpon",
    ...
    reply = asyncio.run(dynamic_operations.replier.generate_reply_for_post(item, character_name="洛天依"))
...
def test_dynamic_replier_targets_comment_in_sender_labeled_message_list():
    ...
    decision = asyncio.run(dynamic_operations.replier.generate_reply_for_comment(item, character_name="洛天依"))
