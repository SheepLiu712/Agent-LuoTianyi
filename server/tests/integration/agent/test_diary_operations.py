"""
DiaryOperations / DiaryTask 单元测试。
"""
import asyncio
import json
import sys
...
def _make_capability_with_llm() -> DiaryOperations:
    """构造带 LLM 配置的 DiaryOperations，并注册 Fake LLM。"""
    capability = DiaryOperations(
        {
            "diary_llm": {
                "llm_module": {
...
def test_diary_prompt_uses_prompt_manager_schema():
    prompt_path = Path(__file__).resolve().parents[3] / "res" / "agent" / "prompts" / "diary_prompt.json"
...
def test_generate_and_post_diary_missing_dynamic_operations():
    capability = _make_capability_with_llm()
    ok, msg, _ = asyncio.run(capability.generate_and_post_diary("user-1"))
    assert ok is False
    assert "DynamicOperations" in msg


def test_generate_and_post_diary_missing_llm():
    capability = DiaryOperations({})
    dynamic_cap = FakeDynamicOperations()
    capability.wire_dependencies(database_manager=None, dynamic_operations=dynamic_cap)
    ok, msg, _ = asyncio.run(capability.generate_and_post_diary("user-1"))
    assert ok is False
    assert "LLM" in msg
...
def test_generate_and_post_diary_full_flow():
    ...
    capability.wire_dependencies(
        ...
        dynamic_operations=dynamic_cap,
    )
...
def test_diary_capability_exposes_llm_readiness():
    ...
    assert DiaryOperations({}).ensure_llm() is False
