...
class Dynamics:
    ...
    def publish_agent_dynamic(self, **kwargs):
        ...
        item = self.by_source.setdefault(
            key,
            {
...
def _fact(owner_user_id: str = "user-1") -> d.DiaryPlanningDue:
    return d.DiaryPlanningDue(
        ...
        trigger_id="diary:luotianyi:2026-09-15",
...
def _skill(diary: DiaryOperations, dynamics: Dynamics) -> DiaryWritingSkill:
    return DiaryWritingSkill(
        diary,
        dynamics,
        {
            "luotianyi": CharacterNarrative(
                name="洛天依",
                persona="温柔的虚拟歌手",
                speaking_style="自然细腻",
            )
...
def test_write_diary_publishes_private_non_commentable_dynamic_with_old_source_id():
    ...
    result = asyncio.run(
        WriteDiaryHandler("luotianyi", _skill(diary, dynamics)).realize(
            action,
            d.ExecutionContext(
                execution_id="e",
                interaction_id="wi",
    ...
    assert result.effect_ref == d.EffectRef(
        kind=d.EffectKind.DYNAMIC_POST,
        effect_id="dynamic-1",
    )
...
def test_diary_source_isolated_by_user_and_same_source_is_idempotent():
    diary = DiaryOperations()
    dynamics = Dynamics()
    handler = WriteDiaryHandler("luotianyi", _skill(diary, dynamics))
    ...
    first = d.WriteDiary(action_id="a1", owner_user_id="user-1", local_date=date(2026, 9, 15), body="第一位用户的日记")
    duplicate = d.WriteDiary(action_id="a2", owner_user_id="user-1", local_date=date(2026, 9, 15), body="重复投递")
    other = d.WriteDiary(action_id="a3", owner_user_id="user-2", local_date=date(2026, 9, 15), body="第二位用户的日记")

    results = [asyncio.run(handler.realize(action, context, None)) for action in (first, duplicate, other)]
...
def test_empty_body_fails_handling_without_plan_or_created_claim():
    handler = DiaryPlanningDueHandler(_skill(DiaryOperations(body="  "), Dynamics()))
    fact = _fact()
    sink = Sink()
...
def test_publish_failure_returns_dependency_unavailable_without_effect():
    result = asyncio.run(
        WriteDiaryHandler(
            "luotianyi",
            _skill(DiaryOperations(), Dynamics(publish_ok=False)),
        ).realize(
            d.WriteDiary(action_id="action", owner_user_id="user-1", local_date=date(2026, 9, 15), body="日记"),
            d.ExecutionContext(
                execution_id="e",
                interaction_id="wi",
                current_interaction_revision=1,
