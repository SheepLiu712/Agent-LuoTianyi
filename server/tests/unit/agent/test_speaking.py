...
async def test_say_generation_failure_ends_message_and_stops_plan(
    runtime_dependencies, chunks, error, code, audio_code
):
    kwargs, _ = runtime_dependencies
...
async def test_waiting_tts_can_be_cancelled_without_blocking_loop(task_cancel):
    ...
    async def consume():
        async with aclosing(
            skill.speak(invocation(cancellation=token), text="文字", tone=d.Tone(value="normal"))
        ) as stream:
...
async def test_early_close_does_not_prefetch_remaining_audio():
    ...
    module = Counting()
    async with aclosing(speaking(module).speak(invocation(), text="文字", tone=d.Tone(value="normal"))) as stream:
        await anext(stream)
    ...
    async def collect(character):
        return [
            ...
            async for chunk in skill.speak(
                invocation(character_id=character), text=character, tone=d.Tone(value="normal")
            )
        ]
...
async def test_server_request_cancellation_releases_lock_and_active_request():
    ...
    async def consume():
        return [
            chunk
            async for chunk in speaking(module).speak(
                invocation(cancellation=token), text="文字", tone=d.Tone(value="normal")
            )
        ]
    ...
    output = [chunk async for chunk in speaking(module).speak(invocation(), text="新文字", tone=d.Tone(value="normal"))]
...
def prepared_manifest(tmp_path):
    ...
    manifest.write_text(
        json.dumps([{"name": "voice", "audio_path": "voice.wav", "text": "资源文字", "expression": ""}]),
        encoding="utf-8",
    )
...
async def test_prepared_say_delivery_and_complete_file(
    runtime_dependencies, prepared_manifest, delivery, text, expression
):
    kwargs, _ = runtime_dependencies
...
async def test_prepared_audio_is_read_at_execution_and_fails_before_output(
    runtime_dependencies, prepared_manifest, failure
):
    kwargs, _ = runtime_dependencies
    manifest, audio = prepared_manifest
    kwargs["config"]["prepared_speech"] = {"characters": {"luotianyi": {"manifest": str(manifest)}}}
    ...
    try:
        ...
        assert report.error_code is (
            d.ExecutionErrorCode.AUDIO_EMPTY if failure == "empty" else d.ExecutionErrorCode.AUDIO_GENERATION_FAILED
        )
        assert sink.values == []
    finally:
        await runtime.shutdown()
...
async def test_prepared_read_cancellation_stops_all_delivery(runtime_dependencies, prepared_manifest, monkeypatch):
    ...
    try:
        ...
        plan = replace(
            plan,
            actions=(replace(plan.actions[0], sound_content=None, prepared_audio_ref=d.MediaRef(media_id="voice")),),
        )
        sink = Sink()
