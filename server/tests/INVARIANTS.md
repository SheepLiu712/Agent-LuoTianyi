...
| G8 去重归各自业务边界 | `integration/stage/test_proactive_due_dispatch.py`、`integration/world/test_world_task_dynamics.py`、`integration/capabilities/test_diary.py`、`integration/websocket/test_websocket_idempotency.py` | claim、来源唯一性、按角色/歌曲/日期幂等分别由 owner 负责 |
...
| §11 | 学歌 | `integration/world/test_world_task_dynamics.py`、`e2e/external/test_capability_singing.py`（显式启用） |
| §12 | 动态互动 | `integration/world/test_world_task_dynamics.py`、`integration/capabilities/test_dynamics.py` |
| §13 | 日记 | `integration/world/test_world_task_diary.py`、`integration/capabilities/test_diary.py` |
...
| §15 | 媒体链 | `integration/websocket/test_websocket_adapter.py`、`integration/stage/test_concurrent_handling.py`、`e2e/external/test_capability_image_understanding.py`（显式启用） |
