...
class RuntimeConfigValidator:
    ...
    CORE_LLM_MODULE_PATHS = {
        ...
        "skill.singing.song_emotion_tagger": "agent_runtime.skills.singing.song_emotion_tagger",
        "skill.diary": "agent_runtime.skills.diary.diary_llm.llm_module",
    }

    CORE_VLM_MODULE_PATHS = {
        "agent_runtime.skills.image_understanding": "agent_runtime.skills.image_understanding.vlm_module",
    }
    ...
    def validate(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "world_disabled": [
                item.to_dict() for item in items if item.scope == "world" and item.status in {"disabled", "warning"}
            ],
        }
    ...
    def _validate_security_config(self, config: dict[str, Any]) -> list[ValidationItem]:
        ...
        if not isinstance(database_config, dict) or "message_token_ttl_seconds" not in database_config:
            return [
                ValidationItem(
                    "core",
                    name,
    ...
    def _validate_client_model_types(self, config: dict[str, Any]) -> list[ValidationItem]:
        ...
        type_requirements, valid_type_count, type_items = self._parse_client_model_types(raw or [])
        result.extend(type_items)
        if valid_type_count:
            result.append(
                ValidationItem(
                    "core",
        ...
        for binding in self._collect_client_model_bindings(config):
            result.extend(self._validate_client_model_binding(binding, type_requirements))
        return result

    @staticmethod
    def _parse_client_model_types(raw: list[Any]):
        result: list[ValidationItem] = []
        ...
        for index, item in enumerate(raw):
            ...
            if not type_id:
                result.append(ValidationItem("core", label, "error", "类型 ID 不能为空"))
                continue
            if type_id in seen_ids:
                result.append(ValidationItem("core", label, "error", f"类型 ID 重复: {type_id}"))
                continue
            ...
            valid_type_count += 1
            json_capable = "是" if item.get("requires_json") else "否"
            thinking_capable = "是" if item.get("requires_thinking") else "否"
            result.append(
                ValidationItem(
                    "core",
                    f"client_model_types.{type_id}",
                    "ok",
                    f"{model_kind.upper()} / JSON={json_capable} / thinking={thinking_capable}",
                )
            )
        return type_requirements, valid_type_count, result

    @staticmethod
    def _validate_client_model_binding(binding, type_requirements):
        path = binding["path"]
    ...
    def _collect_client_model_bindings(self, config: dict[str, Any]) -> list[dict[str, Any]]:
                            ...
                            bindings.append(
                                {
                                    ...
                                    "use_json": bool(value.get("use_json", cfg.get("use_json", False))),
                                }
                            )
    ...
    def _validate_core_resources(self, config: dict[str, Any]) -> list[ValidationItem]:
        ...
        tts_cfg = config.get("agent_runtime", {}).get("skills", {}).get("speaking", {}).get("characters", {})
        if not tts_cfg:
        ...
        sing_cfg = config.get("agent_runtime", {}).get("skills", {}).get("singing", {})
    ...
    def _validate_song_knowledge_resources(self, config: dict[str, Any]) -> list[ValidationItem]:
        ...
        lyric_file = (
            linker_cfg.get("lyric_file") or linker_cfg.get("lyrics_file") or "res/knowledge/song_lyric_keywords.txt"
        )
    ...
    def _validate_world_optionals(self, config: dict[str, Any]) -> list[ValidationItem]:
        ...
        result.append(
            ValidationItem(
                ...
                (
                    "高德地图 key 已配置"
                    if citywalk_key and not str(citywalk_key).startswith("$")
                    else "缺少 AMAP_KEY，将禁用 citywalk"
                ),
                severity="warning",
            )
        )
        ...
        result.append(
            ValidationItem(
                ...
                (
                    "ok"
                    if bili_cookie_path
                    and bili_cookie_path.exists()
                    and bili_cookie_path.read_text(encoding="utf-8-sig").strip()
                    else "disabled"
                ),
                (
                    "B站 cookie 已配置"
        ...
        legacy_qq_path = (
            self._resolve_path(self._get(world, "auto_song_learner.songlearner_resource_dir") or "res/song_learner")
    ...
    def _module_item(self, kind: str, name: str, interface_name: str, available: set[str]) -> ValidationItem:
        ...
        if interface_name not in available:
            return ValidationItem(
                "core", f"{kind}_module.{name}", "error", f"绑定的 interface 不存在: {interface_name}"
            )
