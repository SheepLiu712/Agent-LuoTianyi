...
class OpenAIAPIInterface(
    ...
    def _ensure_client(self) -> "OpenAI":
        ...
        if not self.api_key:
            raise RuntimeError(
                "服务端未配置 VLM API Key（该接口处于客户端模式），" "且本次调用未启用客户端执行，无法完成调用"
            )
        ssl_cert_file = os.environ.get("SSL_CERT_FILE")
        if ssl_cert_file and not os.path.exists(ssl_cert_file):
            self.logger.warning(
                f"检测到 SSL_CERT_FILE 环境变量指向不存在的文件: {ssl_cert_file}。" "正在移除该环境变量以避免错误。"
            )
                ...
                messages = [
                    {
                        ...
                        "content": [
                            ...
                            {"type": "image_url", "image_url": {"url": payload_url, "detail": "auto"}},
                        ],
                    },
                ]
                # 放入线程池执行
