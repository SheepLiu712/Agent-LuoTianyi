...
class LLMAPIInterface(ABC):
    ...
    async def generate_response(
        self, prompt: str, params: Dict[str, Any], enable_thinking: bool = False, use_json: bool = False, **kwargs
    ) -> Dict[str, Any]:
        """
        生成LLM的响应 (异步)
...
class OpenAIAPIInterface(LLMAPIInterface):
    ...
    def _ensure_client(self) -> "OpenAI":
        ...
        if not self.api_key:
            raise RuntimeError(
                "服务端未配置 LLM API Key（该接口处于客户端模式），" "且本次调用未启用客户端执行，无法完成调用"
            )
    ...
    def _extract_content(self, response, elapsed: float = 0.0) -> Dict[str, Any]:
        ...
        return extract_openai_response(response, elapsed=elapsed, logger=self.logger)
    ...
    async def generate_response(self, prompt: str, use_json: bool = False, **kwargs) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                def _do_request():
                    return requests.post(self.url, headers=self.headers, json=self.payload, timeout=10)
