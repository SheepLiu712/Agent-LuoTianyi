...
class DynamicStore:
    ...
    def create_dynamic(
        ...
        *,
        ...
        validation_error = self._validate_dynamic_input(normalized_content, visibility, owner_user_id)
        if validation_error:
        ...
        try:
            if idempotent_by_source and source_id:
                existing = self._find_source_dynamic(db, author_type, author_id, source_type, source_id)
                if existing is not None:
                    return self._existing_dynamic_result(db, existing)

            image_refs_raw = json.dumps(image_refs or [], ensure_ascii=False) if image_refs is not None else None
            memory_status, reply_status = self._default_dynamic_statuses(
                memory_policy,
                author_type,
                memory_status,
                reply_status,
            ...
            created = run_sql_write(
                lambda: self._create_dynamic_in_session(
                    db=db,
            ...
            db.rollback()
            if idempotent_by_source and source_id:
                existing = self._find_source_dynamic(
                    db,
                    author_type,
                    author_id,
                    source_type,
    ...
    @staticmethod
    def _validate_dynamic_input(content: str, visibility: str, owner_user_id: str | None) -> str:
        if not content:
            return "动态内容不能为空"
    ...
    @staticmethod
    def _default_dynamic_statuses(memory_policy, author_type, memory_status, reply_status):
        return (
            memory_status if memory_status is not None else ("pending" if memory_policy == "candidate" else "disabled"),
            reply_status if reply_status is not None else ("pending" if author_type == "user" else "not_applicable"),
        )

    @staticmethod
    def _find_source_dynamic(
        db,
        author_type,
        author_id,
        source_type,
        ...
        *,
        ...
        query = db.query(DynamicPost).filter(
            DynamicPost.author_type == author_type,
            DynamicPost.author_id == author_id,
            DynamicPost.source_type == source_type,
            DynamicPost.source_id == source_id,
        ...
        return query.order_by(DynamicPost.created_at.asc(), DynamicPost.id.asc()).first()

    def _existing_dynamic_result(self, db, dynamic):
        user_names = self._load_user_names(
            db,
            {dynamic.author_id} if dynamic.author_type == "user" else set(),
        )
        return True, "dynamic already exists", self._serialize_dynamic_post(dynamic, user_names=user_names)

    def _create_dynamic_in_session(self, *, db, **values):
        owner_user_id = values["owner_user_id"]
        if owner_user_id and not self.user_store.user_exists_in_session(db, owner_user_id):
            return None
        created = DynamicPost(**values, status="published")
        db.add(created)
        db.flush()
        if values["author_type"] != "user":
            self._clamp_dynamic_read_states(db, created.created_at or datetime.now())
        db.commit()
        return created

    def _clamp_dynamic_read_states(self, db, created_at):
        for user_id in self.user_store.list_user_ids_in_session(db):
            state = self._get_or_create_dynamic_read_state(db, user_id)
            if state.last_read_dynamic_at is not None and state.last_read_dynamic_at > created_at:
                state.last_read_dynamic_at = created_at
    ...
    def list_dynamics_for_user(
        ...
        *,
        ...
        try:
            ...
            items = [self._serialize_dynamic_post(row, user_names, comment_counts.get(row.id, 0)) for row in page_rows]
    ...
    def list_dynamic_comments_for_user(
        ...
        *,
        ...
        try:
            ...
            rows = query.order_by(DynamicComment.created_at.asc(), DynamicComment.id.asc()).limit(page_size + 1).all()
            has_more = len(rows) > page_size
            page_rows = rows[:page_size]
            user_names = self._load_user_names(
                db,
    ...
    def create_dynamic_comment(
        ...
        *,
        ...
        try:
            ...
            created = run_sql_write(
                lambda: self._create_comment_in_session(
                    db=db,
    ...
    def _create_comment_in_session(self, *, db, **values):
        dynamic = db.query(DynamicPost).filter(DynamicPost.id == values["dynamic_id"]).first()
        if not self._can_comment(dynamic, values["author_type"], values["owner_user_id"]):
            return None
        if not self._valid_parent_comment(
            db,
            ...
            return None
        if not self.user_store.user_exists_in_session(db, values["owner_user_id"]):
            return None
        created = DynamicComment(**values, status="published")
        db.add(created)
        db.flush()
        db.commit()
        return created
    ...
    @staticmethod
    def _can_comment(dynamic, author_type, owner_user_id):
        if dynamic is None or dynamic.status != "published" or not dynamic.allow_comment:
            return False
        if author_type == "user" and not _dynamic_is_visible_to_user(dynamic, owner_user_id):
            return False
        return dynamic.visibility != "private" or dynamic.owner_user_id == owner_user_id

    @staticmethod
    def _valid_parent_comment(db, parent_comment_id, dynamic_id, owner_user_id):
        if not parent_comment_id:
            return True
        parent = db.query(DynamicComment).filter(DynamicComment.id == parent_comment_id).first()
        return parent is not None and parent.dynamic_id == dynamic_id and parent.owner_user_id == owner_user_id
