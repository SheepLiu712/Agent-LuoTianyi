...
class CredentialService:
    ...
    def get_user_uuid_by_username(self, username: str) -> Optional[str]:
        """
        根据用户名获取用户 UUID。优先读取 Redis 的 user_id:{username} 缓存；
        缓存未命中时查询数据库并回填缓存。
        """
    ...
    def check_auth_token(self, username: str, token: str) -> bool:
        ...
        try:
            user = db.query(User).filter_by(username=username).first()
            return bool(user and user.auth_token and token and hmac.compare_digest(user.auth_token, token))
        finally:
            db.close()
    ...
    def check_message_token(self, username: str, token: str) -> Tuple[bool, Optional[str]]:
        ...
        db = self._new_session()
        try:
            user = db.query(User).filter(User.uuid == user_uuid, User.username == username).first()
            if not user or not user.auth_token:
                return False, None
    ...
    def reset_account(self, invite_code_str: str, new_username: str, new_password: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            existing = db.query(User).filter(User.username == new_username, User.uuid != user.uuid).first()
            if existing:
    ...
    def admin_list_invite_codes(
        ...
        *,
        ...
        try:
            ...
            total = query.count()
            rows = query.order_by(InviteCode.created_at.desc()).offset(offset).limit(limit).all()
    ...
    def _rotate_authenticated_session(
        ...
        *,
        ...
        db = self._new_session()
        try:
            updated = db.query(User).filter(*filters).update(updates, synchronize_session=False)
            if updated != 1:
                db.rollback()
                return None
            db.commit()
