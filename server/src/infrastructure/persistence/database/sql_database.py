...
class User(Base):
    ...
    dynamic_read_state = relationship(
        "DynamicReadState", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
...
class Event(Base):
    ...
    )  # concert / livestream / holiday / birthday / anniversary / travel / new_song / dynamic / general
...
def _migrate_sqlite_schema(db_engine: Engine) -> None:
    ...
    with db_engine.begin() as connection:
        ...
        _migrate_event_schema(connection)
...
def _migrate_conversation_schema(connection) -> None:
    if "character_id" not in _table_columns(connection, "conversations"):
...
def _migrate_event_schema(connection) -> None:
    if "character" not in _table_columns(connection, "events"):
        connection.exec_driver_sql("ALTER TABLE events ADD COLUMN character VARCHAR NOT NULL DEFAULT 'luotianyi'")
    connection.exec_driver_sql("CREATE INDEX IF NOT EXISTS ix_events_character ON events (character)")


def _migrate_notification_schema(connection) -> None:
    columns = _table_columns(connection, "event_notifications")
    if columns and "character_id" not in columns:
    ...
    if columns:
        for name, column in (
            ...
            connection.exec_driver_sql(
                f"CREATE INDEX IF NOT EXISTS ix_event_notifications_{name} ON event_notifications ({column})"
            )


def _migrate_dynamic_schema(connection) -> None:
    post_columns = _table_columns(connection, "dynamic_posts")
    if post_columns:
        if "memory_error" not in post_columns:
            connection.exec_driver_sql("ALTER TABLE dynamic_posts ADD COLUMN memory_error TEXT")
        if "reply_error" not in post_columns:
        ...
        if duplicate_diaries:
            raise RuntimeError(
                "Cannot create diary idempotency index: duplicate diary source keys exist: " f"{duplicate_diaries}"
            )
        connection.exec_driver_sql("""
                ...
                WHERE source_type = 'diary' AND source_id IS NOT NULL
                """)
    comment_columns = _table_columns(connection, "dynamic_comments")
    if comment_columns:
        if "memory_error" not in comment_columns:
            connection.exec_driver_sql("ALTER TABLE dynamic_comments ADD COLUMN memory_error TEXT")
        if "reply_error" not in comment_columns:
            connection.exec_driver_sql("ALTER TABLE dynamic_comments ADD COLUMN reply_error TEXT")


def _migrate_invite_schema(connection) -> None:
    columns = _table_columns(connection, "invite_codes")
    if columns and "disabled" not in columns:
        connection.exec_driver_sql("ALTER TABLE invite_codes ADD COLUMN disabled BOOLEAN NOT NULL DEFAULT 0")
    connection.exec_driver_sql("""
            ...
            """)
    migration_name = "2026-08-22-enable-all-invite-codes"
