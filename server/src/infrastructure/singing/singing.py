...
class SingingBackend:
    """Shared song-library and audio-rendering backend used by Agent skills."""
    ...
    def __init__(self, config: Dict[str, Any], llm_service=None) -> None:
        ...
        if "characters" in config:
            ...
            raise ValueError("infrastructure.sing requires a 'characters' layer for character song resources.")
        if not isinstance(characters_config, dict) or not characters_config:
            raise ValueError("infrastructure.sing.characters must contain at least one character.")
        ...
        self.default_character_id = (
            CharacterName.LUOTIANYI.value
            if CharacterName.LUOTIANYI.value in self.singing_manager
            else next(
    ...
    def ensure_dependencies(self) -> None:
        ...
        if missing:
            raise RuntimeError(f"SingingBackend dependencies are missing: {', '.join(missing)}")
    ...
    def can_i_sing_song(self, character_id: str, song_name: str) -> Tuple[str, List[str]]:
        ...
        :return: 歌曲的正确名称和可演唱段落列表；不能演唱时返回空歌名和空列表。
        """
