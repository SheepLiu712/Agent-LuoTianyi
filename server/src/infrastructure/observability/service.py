...
class ObservabilityService:
    ...
    def get_pipeline_latency_summary(self, *, days: int = 7) -> Dict[str, Any]:
        ...
        return {span_name: self._summarize_values(values) for span_name, values in grouped.items()}
    ...
    def _summarize_llm_rows_by_day(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        return [{"day": day, **self._summarize_llm_metric_rows(day_rows)} for day, day_rows in sorted(grouped.items())]
    ...
    def get_trace_detail(self, trace_id: str) -> dict[str, Any]:
        summaries = [
            row
            for row in self.get_trace_summaries(days=self.retention_days, limit=10000)
    ...
    def get_memory_trace_summary(self, *, days: int = 7) -> dict[str, Any]:
        ...
        totals = self._query_one(
            ...
            SELECT
                ...
                SUM(
                    CASE WHEN annotation_required = 1 AND a.event_id IS NULL THEN 1 ELSE 0 END
                ) AS pending_annotation_count,
        ...
        by_type = self._query_all(
            ...
            SELECT e.event_type, e.item_type,
                COUNT(*) AS event_count,
                SUM(
                    CASE WHEN e.annotation_required = 1 AND a.event_id IS NULL THEN 1 ELSE 0 END
                ) AS pending_annotation_count,
