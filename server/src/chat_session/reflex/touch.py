...
class TouchFastReplyBuilder:
    ...
    def __init__(self, config: Mapping, touch_voice_dir: Path | None = None):
        self.config = dict(config)
        server_root = Path(__file__).resolve().parents[2]
        configured_dir = self.config.get("touch_voice_dir")
        if touch_voice_dir is not None:
            self.touch_voice_dir = touch_voice_dir
        elif configured_dir:
            self.touch_voice_dir = Path(configured_dir)
        else:
            self.touch_voice_dir = server_root / "res" / "agent" / "touch_voice"
