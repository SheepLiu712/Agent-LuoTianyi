...
def get_model_from_config(model_type: str, config_path: str) -> Tuple[nn.Module, Union[ConfigDict, OmegaConf]]:
    ...
    if model_type == 'mdx23c':
        ...
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer import MelBandRoformer
        model = MelBandRoformer(**dict(config.model))
    elif model_type == 'mel_band_conformer':
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer import MelBandConformer
        model = MelBandConformer(**dict(config.model))
    elif model_type == 'mel_band_roformer_experimental':
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer.mel_band_roformer_experimental import MelBandRoformer
        model = MelBandRoformer(**dict(config.model))
    elif model_type == 'bs_roformer':
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer import BSRoformer
        model = BSRoformer(**dict(config.model))
    elif model_type == 'bs_conformer':
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer import BSConformer
        model = BSConformer(**dict(config.model))
    elif model_type == 'bs_roformer_experimental':
        from src.world.get_new_songs.song_learner.src.msst_core.models.bs_roformer.bs_roformer_experimental import BSRoformer
        model = BSRoformer(**dict(config.model))
