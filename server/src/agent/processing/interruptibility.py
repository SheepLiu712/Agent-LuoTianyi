...
class _InteractionInterruptibility:
    ...
    def allows(self, interaction_id: str, operation: str, request_id: str | None = None) -> bool:
        if not isinstance(interaction_id, str) or not interaction_id.strip():
            raise ValueError("interaction_id must be nonblank")
        calls = self._active.get((interaction_id, operation), ())
        calls = tuple(call for call in calls if request_id is None or call.request_id == request_id)
