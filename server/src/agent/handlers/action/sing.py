...
class SingHandler:
    ...
    async def realize(self, action: d.Action, execution_context: d.ExecutionContext,
        ...
        try:
            ...
            return await self._failed(action, outputs, delivery,
                                      d.AudioErrorCode.EMPTY_AUDIO, d.ExecutionErrorCode.AUDIO_EMPTY)
        except TimeoutError:
            get_logger(__name__).exception(
                f"SING timed out character_id={self._character_id} action_id={action.action_id}")
            return await self._failed(action, outputs, delivery,
                                      d.AudioErrorCode.GENERATION_FAILED, d.ExecutionErrorCode.PROVIDER_TIMEOUT)
        except Exception:
            get_logger(__name__).exception(
                f"SING generation failed character_id={self._character_id} action_id={action.action_id}")
            return await self._failed(action, outputs, delivery,
                                      d.AudioErrorCode.GENERATION_FAILED, d.ExecutionErrorCode.AUDIO_GENERATION_FAILED)
        if execution_context.cancellation.is_cancelled:
            return self._result(action, d.ExecutionErrorCode.CANCELLED)
        await outputs.emit(AudioChunkDraft(delivery=delivery, data=audio,
                                           framing=d.AudioFraming.COMPLETE_FILE))
        await outputs.emit(MessageEndDraft(delivery=delivery, status=d.MessageEndStatus.COMPLETED,
                                           error_code=None))
        return self._result(action)

    async def _failed(self, action: d.Sing, outputs: OutputEmitter, delivery: d.OutputDelivery,
                      audio_code: d.AudioErrorCode, error: d.ExecutionErrorCode) -> d.ActionResult:
        await outputs.emit(MessageEndDraft(delivery=delivery, status=d.MessageEndStatus.FAILED,
                                           error_code=audio_code))
