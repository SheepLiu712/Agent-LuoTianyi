...
class DiaryOperations:
    ...
    核心流程：
      ...
      5. 通过 DynamicOperations.publish_agent_dynamic() 发布为 Agent 动态

    复用了动态系统的完整链路：
      DiaryWritingSkill → DynamicOperations → DynamicStore → 用户可见
    """
    ...
    def __init__(self, config: Dict[str, Any] | None = None) -> None:
        ...
        self._dynamic_operations: "DynamicOperations | None" = None
    ...
    def wire_dependencies(
        ...
        *,
        ...
        dynamic_operations: Any | None = None,
    ) -> None:
        """
        注入外部依赖。

        由 SharedSkills 在初始化阶段调用。
        dynamic_operations 是日记的发布通道——日记本质是一种特殊类型的动态。
        """
        self.database_manager = database_manager
        self._dynamic_operations = dynamic_operations
    ...
    def ensure_dependencies(self) -> Tuple[bool, str]:
        ...
        if self._dynamic_operations is None:
            self.logger.warning("DynamicOperations is not available")
            return False, "DynamicOperations 不可用"
    ...
    async def generate_and_post_diary(
        ...
        *,
        ...
        """
        生成日记并通过 DynamicOperations 发布为 Agent 动态。

        执行顺序：
          1. 前置检查（DynamicOperations、LLM、数据库是否可用）
          2. 获取用户信息和角色人设
          3. 收集今日聊天记录和用户动态作为素材
        ...
        # ── 通过 DynamicOperations 发布为 Agent 动态 ──
        try:
            # publish_agent_dynamic 会创建一条 author_type=agent 的动态，
            # 日记会被保存到 dynamic_posts 表，用户通过动态界面即可查看。
            ok, msg, item = self._dynamic_operations.publish_agent_dynamic(
                character_id=character_id,
                content=diary_text,
            ...
            self.logger.error(f"DynamicOperations call failed: {exc}")
    ...
    async def _collect_materials(
        ...
        lines: List[str] = []
        try:
            lines.extend(self._collect_conversation_materials(user_id, character_id, target_date))
        except Exception as exc:
            self.logger.warning(f"Failed to collect conversations for diary: {exc}")
        try:
            lines.extend(self._collect_dynamic_materials(user_id, target_date))
        except Exception as exc:
            self.logger.warning(f"Failed to collect dynamics for diary: {exc}")
        return "\n".join(lines) if lines else "今天没有特别的互动记录。"

    def _collect_conversation_materials(
        self,
        user_id: str,
        character_id: str,
        target_date: str,
        ...
        """读取并格式化当天最近的对话材料。"""
        service = self.database_manager.conversation_service
        total = service.get_total_conversation_count(user_id, character_id=character_id)
        if total <= 0:
            return []
        conversations = service.get_history_from_db(
            user_id,
            max(0, total - 100),
            total,
            character_id=character_id,
        )
        today = [item for item in conversations if item.timestamp.startswith(target_date)]
        if not today:
            return []
        return [
            "【今日聊天记录】",
            *(f"[{'用户' if item.source == 'user' else '天依'}] {item.content}" for item in today[-30:]),
        ]

    def _collect_dynamic_materials(self, user_id: str, target_date: str) -> list[str]:
        """读取并格式化当天由用户发布的动态材料。"""
        store = self.database_manager.dynamic_store
        if store is None:
            return []
        items = store.list_dynamics_for_user(user_id, limit=50).get("items", [])
        today = [
            item
            for item in items
            if item.get("created_at", "").startswith(target_date) and item.get("author_type") == "user"
        ]
        if not today:
            return []
        return ["\n【今日用户动态】", *(f"[用户] {item.get('content', '')}" for item in today)]
    ...
    def _parse_diary_result(self, raw: str, target_date: Optional[str] = None) -> Optional[str]:
        ...
        mood, body_lines = self._parse_diary_lines(raw.strip().split("\n"))
    ...
    @staticmethod
    def _parse_diary_lines(lines: list[str]) -> tuple[str, list[str]]:
        """提取心情和正文行，同时兼容旧版标题与摘要。"""
        mood = ""
        body_lines: list[str] = []
        in_body = False
        for line in lines:
