...
class SpeechBackend:
    """Shared TTS worker registry and lifecycle owner."""
    ...
    def ensure_dependencies(self) -> None:
        ...
        if self.tts_config is None:
            raise RuntimeError("SpeechBackend dependency is missing: tts_config")
        if self.tts_module is None:
            raise RuntimeError("SpeechBackend dependency is missing: tts_module")
    ...
    # Reviewed exception: shutdown must keep retry, cancellation, timeout, and
    # error aggregation in one atomic lifecycle transition.
