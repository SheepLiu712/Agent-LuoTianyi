...
# Reviewed exception: this process entry point owns the worker protocol and its
# complete startup/request/shutdown state machine.
def _run_gsv_worker(  # noqa: C901
    config_path: str,
...
class TTSServer:
        ...
        with self._lifecycle_lock:
            try:
                ...
                try:
                    ...
                    self.logger.error(f"Failed to roll back gsv_tts startup: {cleanup_error}")
                raise
    ...
    # Reviewed exception: process and queue teardown is a single idempotent
    # lifecycle transition whose ordering must remain visible in one place.
    def _stop(self, force: bool = False) -> None:  # noqa: C901
        self.request_stop()
        process = self.server_process

        if process is not None:
            if not force and not self._wait_for_active_requests(timeout=10.0):
                self.logger.warning("Timed out waiting for active gsv_tts requests; forcing worker shutdown")
                force = True
    ...
    def _wait_for_response(
        self, request_id: str, timeout: int, cancel_event: threading.Event | None = None
    ) -> Dict[str, Any]:
