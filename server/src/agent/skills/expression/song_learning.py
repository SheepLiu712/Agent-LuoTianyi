...
class SongLearningDispatchSkill:
    ...
    def __init__(self, singing_manager: _SingingManagerAdapter | None) -> None:
        if singing_manager is not None:
            missing = tuple(
                name
                for name in (
                    "add_wished_song",
                ...
                if not callable(getattr(singing_manager, name, None))
            )
            if missing:
                raise TypeError(f"唱歌管理器缺少可调用方法: {', '.join(missing)}")
    ...
    def request(self, *, song_id: str) -> bool:
        ...
        return bool(self._singing.add_wished_song(song_id))
    ...
    def material(self, *, song_id: str) -> tuple[str, str]:
        ...
        try:
            resolved, found = self._singing.can_i_sing_song(song_id)
        ...
        lyrics = ""
        try:
            lyrics = str(self._singing.get_full_lyrics(correct_name) or "").strip()
        except Exception:  # noqa: BLE001 - 材料缺失不应阻断学歌事实的结算
            lyrics = ""
        if not lyrics and segment_description:
            try:
                lyrics = str(self._singing.get_segment_lyrics(correct_name, segment_description) or "").strip()
            except Exception:  # noqa: BLE001 - 材料缺失不应阻断学歌事实的结算
                lyrics = ""
