...
class DynamicReplier:
    ...
    async def generate_reply_for_post(self, item: dict[str, Any], *, character_name: str) -> str:
        """为动态帖子生成回复文本。
    ...
    async def generate_reply_for_comment(self, item: dict[str, Any], *, character_name: str) -> dict[str, Any]:
        """为动态评论生成是否回复的判断及回复文本。
    ...
    def _build_message_context(
        ...
        *,
        ...
        formatted_messages = [
            cls._format_message(message, index, character_name) for index, message in enumerate(messages, start=1)
        ]
