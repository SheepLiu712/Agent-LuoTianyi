...
class _Mind(Protocol):
    async def write_topic_memories(self, user_id: str, current_dialogue: str,
                                   related_memories=None, conversation_history=None): ...
...
class ReflectionSkill:
    ...
    async def consolidate_memories(self, *, character_id: str, user_id: str, current_dialogue: str,
                                   conversation_history: str = "",
                                   related_memories: list[str] | None = None) -> dict[str, Any]:
        """依据本次已结算对话提取并写入长期记忆，返回底层提交结果。"""
        runtime = self._runtime_provider(character_id)
        return await runtime.mind.write_topic_memories(
            user_id=user_id, current_dialogue=current_dialogue,
            related_memories=related_memories, conversation_history=conversation_history)

    async def update_profile(self, *, character_id: str, user_id: str, summary: str,
                             recent_conversation: list[str]) -> str | None:
