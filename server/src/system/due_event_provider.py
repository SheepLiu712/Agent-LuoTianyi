...
from src.stage.due_events import DueEvent
...
class EventStoreDueEventProvider:
    ...
    def list_due(
        ...
        for event, trigger_key in self._event_store.get_events_due_for_trigger(
            character=character_id, today=now.date(),
        ):
            ...
            values.append(DueEvent(
                ...
                is_notified=self._event_store.is_notified(
                    event_id, user_id, str(trigger_key), character_id,
                ),
            ))
        return tuple(values)
    ...
    def claim(
        ...
        return self._event_store.try_claim_notification(
            event_id, user_id, trigger_key, character_id,
        )
