...
class AdminAuthError(Exception):
    """Authentication policy failure independent of any transport."""

    def __init__(self, status_code: int, detail: dict[str, str]) -> None:
...
class AdminAuthService:
    ...
    def setup(self, setup_token: str, password: str) -> dict[str, Any]:
        if self.is_configured():
            raise AdminAuthError(
                status_code=400,
                detail={"code": "ADMIN_ALREADY_CONFIGURED", "message": "Admin password is already configured"},
            )
        expected = self._ensure_setup_token()
        if not hmac.compare_digest(setup_token.strip(), expected.strip()):
            raise AdminAuthError(
                status_code=403, detail={"code": "BAD_SETUP_TOKEN", "message": "Invalid admin setup token"}
            )
    ...
    def login(self, password: str) -> dict[str, Any]:
        if not self.is_configured():
            raise AdminAuthError(
                status_code=403,
                detail={"code": "ADMIN_SETUP_REQUIRED", "message": "Admin password has not been configured"},
            )
        if not self._verify_password(password):
            raise AdminAuthError(
                status_code=401, detail={"code": "BAD_ADMIN_PASSWORD", "message": "Invalid admin password"}
            )
        return self._create_session()

    async def login_async(self, password: str) -> dict[str, Any]:
        if not self.is_configured():
            raise AdminAuthError(
                status_code=403,
                detail={"code": "ADMIN_SETUP_REQUIRED", "message": "Admin password has not been configured"},
        ...
        try:
            ...
            raise AdminAuthError(
                status_code=503, detail={"code": "ADMIN_AUTH_BUSY", "message": "Admin authentication is busy"}
            ) from exc
        try:
    ...
    def require_admin(self, token: str | None) -> None:
        if not token:
            raise AdminAuthError(
                status_code=401, detail={"code": "ADMIN_AUTH_REQUIRED", "message": "Admin authentication required"}
            )
        expires_at = self.sessions.get(token)
        if not expires_at or expires_at < time.time():
            self.sessions.pop(token, None)
            raise AdminAuthError(
                status_code=401, detail={"code": "ADMIN_SESSION_EXPIRED", "message": "Admin session expired"}
            )
    ...
    def _write_password(self, password: str) -> None:
        ...
        if len(password) < 8:
            raise AdminAuthError(
                status_code=400,
                detail={"code": "WEAK_ADMIN_PASSWORD", "message": "Admin password must be at least 8 characters"},
            )
