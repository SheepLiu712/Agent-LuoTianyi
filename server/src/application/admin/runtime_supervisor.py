...
from src.infrastructure.observability import ObservabilityService, new_trace_id
from src.utils.logger import get_logger


class RuntimeSupervisor:
    """Owns the business ServerRuntime lifecycle behind the admin shell."""
    ...
    async def start(self) -> dict[str, Any]:
        async with self._lock:
            ...
            try:
                ...
                from src.server_runtime import ServerRuntime, set_server_runtime

                runtime = await ServerRuntime.initialize(config, observability=self.observability)
                set_server_runtime(runtime)
    ...
    async def stop(self) -> dict[str, Any]:
        async with self._lock:
            ...
            self._clear_error()
            try:
                from src.server_runtime import set_server_runtime

                set_server_runtime(None)
