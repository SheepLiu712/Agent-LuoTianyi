...
from src.infrastructure.persistence.database import InviteCode, User
...
class PasswordDecryptionError(ValueError):
    """Encrypted login material could not be decoded with the active key."""
...
def decrypt_password(encrypted_b64: str) -> str:
    try:
        ...
        raise PasswordDecryptionError("Encryption error") from e
...
def reset_account(db_session: Session, invite_code_str: str, new_username: str, new_password: str) -> Tuple[bool, str]:
    """以邀请码重置账号的用户名和密码。
    ...
    # 检查新用户名是否已被其他用户使用
    existing = db_session.query(User).filter(User.username == new_username, User.uuid != user.uuid).first()
    if existing:
