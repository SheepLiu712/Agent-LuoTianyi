...
3. 下载资源：
   ...
   - 如需迁移数据库，将 `data` 文件夹解压到 `server` 根目录。
...
2. 打开控制台地址。首次进入需要初始化管理员密码，按页面提示输入 `config/admin_setup_token.txt` 中的 setup token。

3. 进入“服务配置”页面，先配置环境变量 / Secrets：
   - `JWT_SECRET`：必需，用于服务端鉴权和加密相关逻辑。
   - `AMAP_KEY`：必需，高德地图 API Key。
...
7. 如果需要公网访问，再打开 sakurafrp 等内网穿透工具，将 `60030` 端口暴露出去。控制台已启用管理员密码，请妥善保管。
...
- [ ] v0.3.x: 动态功能更新
  ...
  - [ ] 在动态界面，用户可以主动发布动态；
  - [ ] 天依可以根据用户的动态进行评论，并进入记忆库；
  - [ ] 天依在完成城市漫步和学习完新歌曲后，可以主动发布动态；
- [ ] v0.4.x: 记忆和知识库的更新
  - [ ] 重构记忆的保存机制，构建知识图谱，使得不同记忆之间实现联想；
  - [ ] 进一步优化记忆的检索和保存，提升命中率和联想能力；
  - [ ] 增加做梦任务，使得天依可以在睡觉时进行记忆的整理和联想；
  - [ ] 增加上下文注意力机制，使得天依可以根据规划关系、心情、日程、记忆、前文对回复的影响进行更合理的回复；
- [ ] v1.0.x：多角色更新
  - [ ] 委托画师和建模师，制作乐正绫的Live2D模型；
  - [ ] 修改服务端的配置，使得可以同时支持多个角色；
  - [ ] 构建与AgentLuo客户端完全相同的AgentLing客户端，用户可以在两个APP中分别与天依和阿绫进行互动；
