...
from cli_client.network.event_types import WSEventType, WSMessage, normalize_agent_message
from cli_client.session import (
    AggregatedReply,
    HeadlessSession,
    ReplyTimeoutError,
    SessionClosedError,
...
def _executor(session):
    executor = ActionExecutor(session_factory=lambda **_kwargs: session, session_id="session-next")
...
def test_reply_wait_without_uuid_waits_for_next_reply():
    ...
    record, exit_code = executor.execute({"action": "reply.wait", "params": {"timeout": 7, "non_empty": True}})
...
def test_reply_wait_with_uuid_keeps_explicit_semantics():
    ...
    record, exit_code = executor.execute({"action": "reply.wait", "params": {"reply_uuid": "r-x", "timeout": 3}})
...
def test_reply_wait_next_timeout_maps_to_timeout_exit_code():
    ...
    record, exit_code = executor.execute({"action": "reply.wait", "params": {"timeout": 1}})
