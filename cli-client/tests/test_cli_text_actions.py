...
from cli_client.cli.actions import ActionExecutor, ExitCode, parse_action
from cli_client.cli.main import main
from cli_client.cli.output import Redactor, serialize_record
...
def test_jsonl_record_has_stable_schema_and_error_object():
    ...
    record, exit_code = executor.execute({"action": "unknown.action", "action_id": "a-1", "params": {}})
...
def test_connect_failure_is_auth_transport_error_and_redacted():
    secret = "login-secret"
    executor, _session = _executor(FakeSession(connect_error=SessionConnectionError(f"bad password {secret}")))
