...
from cli_client.session import HeadlessSession, SessionNotReadyError, SessionState
...
def test_preferences_update_replace_mode_overwrites_exactly():
    ...
    record, exit_code = _action(executor, "preferences.update", values={"a": 1}, replace=True)
...
def test_preferences_update_requires_values_object(values):
    ...
    record, exit_code = executor.execute({"action": "preferences.update", "params": params})
...
def test_preferences_update_rejects_non_boolean_replace():
    ...
    record, exit_code = _action(executor, "preferences.update", values={"a": 1}, replace="yes")
