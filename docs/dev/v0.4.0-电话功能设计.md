...
<<<<<<< HEAD
4. Live2D HTML 提供 `setExpression`、`feedAudioPacket` 和 `stopAudioStream`，按显式 `stream_type` 隔离 Chat/Call，使播放完成、打断停止都能准确 ACK。
=======
...
<<<<<<< HEAD
- `app/public/live2d/live2d.html` 提供 `window.setExpression()`，音频状态机由 `audio_stream_player.js` 暴露的 `feedAudioPacket` / `stopAudioStream` 接口承载。
=======
...
| `app/public/live2d/live2d.html` | 修改 | 暴露显式 Chat/Call 音频包接口并驱动口型 |
| `app/public/live2d/audio_stream_player.js` | 新增 | 分离 Chat/Call 时间轴、完成/停止终态与取消防护 |
| `app/plugins/withLive2DAndroidAssets.js` | 复用 | 构建时把整个 `app/public/` 同步进 Release APK |
...
=======
| `app/components/call/CallConnectingOverlay.tsx` | 新增 | 灰色低透明度磨砂蒙版和状态文字 |
| `app/components/call/CallHangupButton.tsx` | 新增 | 红色圆形挂断按钮；P0 不依赖图标素材 |
| `app/hooks/useCallLogic.ts` | 新增 | 通话状态机、权限、WebSocket、采集、播放、重连和清理 |
| `app/types/call.ts` | 新增 | 客户端状态、事件 envelope、payload 和错误类型 |
| `app/utils/call_ws_transport.ts` | 新增 | `/call_ws` 连接、鉴权、心跳、快速重连、事件收发 |
| `app/utils/call_audio_controller.ts` | 新增 | 按 `audio_id` 缓冲并严格顺序送入 WebView；完成/停止回调 |
| `app/utils/live2d_webview.ts` | 新增 | 统一 Dev/Release Live2D URL、白名单和注入脚本构造 |
| `app/utils/live2d_helper.ts` | 修改 | 保留表情映射；复用新的注入脚本工具 |
...
| `app/modules/pcm-recorder/*` | 新增 | Android 原生 PCM 采集和重采样 Expo Module |
| `app/package.json` | 修改 | 增加 `expo-blur` 和本地 PCM 模块依赖/配置 |
| `app/app.json` | 视实现修改 | 注册本地 Expo Module 或相关插件配置 |
>>>>>>> dev
...
| `server/res/admin_ui/src/main.tsx` | 修改 | 增加通话记录菜单和摘要、状态、退出码、后处理状态页面 |
| `server/res/admin_ui/src/styles.css` | 修改 | 通话记录表格、状态徽标和详情布局 |
...
=======
通话页和聊天页都通过 `app/utils/live2d_webview.ts` 获取 URL：

```ts
export function getLive2DUrl(): string;
export function isAllowedLive2DUrl(url: string): boolean;
export function buildFeedAudioScript(packet: Live2DAudioPacket): string;
export function buildStopAudioScript(audioId?: string): string;
>>>>>>> dev
```
...
window.feedAudioPacket({ stream_type, audio, is_final, audio_id, response_id })
window.stopAudioStream({ stream_type, audio_ids, response_id, reason })
=======
WebView 对外接口调整为：

```js
window.setExpression(expressionId)
window.feedAudioChunk(base64Audio, isFinal, audioId)
window.stopAudioPlayback(audioId)
>>>>>>> dev
```
...
<<<<<<< HEAD
{"type":"audio_finished","stream_type":"call","audio_id":"audio-...","response_id":"response-..."}
{"type":"audio_stopped","stream_type":"call","audio_id":"audio-...","response_id":"response-...","reason":"user_barge_in"}
```

播放器必须分别保存 Chat/Call 的 `AudioBufferSourceNode` 集合。`stopAudioStream()` 只停止命令指定的流和音频，清理对应时间轴并为未完成的 `audio_id` 发出 `audio_stopped`。停止和完成事件对同一个 `audio_id` 至多发出一个。

聊天与电话包都必须显式传 `stream_type` 和真实 `audio_id`；聊天使用消息 `uuid` 作为 `audio_id`。播放器为 `chat`、`call` 分别维护时间轴和终态，两个客户端只消费属于自己的完成/停止事件。

开发源为 `app/public/live2d/live2d.html` 和同目录的 `audio_stream_player.js`。Android Gradle 的 `syncLive2DPublicAssets` 会在构建时同步整个 `app/public/`，不维护手工复制的 Release 镜像。

Release 验证必须同时检查 APK 内 `assets/public/live2d/live2d.html` 与 `assets/public/live2d/audio_stream_player.js`，不能只在 Expo Dev Server 验证。
...
=======
{"type":"audio_finished","audioId":"audio-..."}
{"type":"audio_stopped","audioId":"audio-..."}
```

HTML 必须保存当前 `AudioBufferSourceNode` 集合。`stopAudioPlayback()` 对匹配 `audioId` 的 source 调用 `stop()`，清空时间轴，关闭口型并发出 `audio_stopped`。停止和完成事件对同一个 `audioId` 至多发出一个。
...
<<<<<<< HEAD
=======
interface UseCallLogicResult {
  session: CallSessionView;
  elapsedSeconds: number;
  live2dReady: boolean;
  setLive2dReady(value: boolean): void;
  hangup(): Promise<void>;
...
=======
`expo-av` 适合录制文件和普通播放，不提供本设计所需的稳定、低延迟原始 PCM 回调。新增本地 Expo Module：

```text
app/modules/pcm-recorder/
  expo-module.config.json
  package.json
  src/index.ts
  src/PcmRecorder.types.ts
  android/build.gradle
  android/src/main/AndroidManifest.xml
  android/src/main/java/expo/modules/pcmrecorder/PcmRecorderModule.kt
>>>>>>> dev
```
...
export async function start(options: PcmRecorderOptions): Promise<void>;
export async function stop(): Promise<void>;
export function addAudioFrameListener(
  listener: (event: { seq: number; audio: string; capturedAt: number }) => void,
): EventSubscription;
```

Android 使用 `AudioRecord`：优先直接请求 PCM16/16 kHz/mono；设备不支持时以设备可用采样率捕获，在 native 线程做单声道转换和线性重采样，再向 JS 发 Base64。40 ms 一帧等于 640 samples、1280 bytes 原始 PCM，Base64 后约 1708 bytes，不加 WAV 头。
>>>>>>> dev
...
`useCallLogic` 将 `call.audio.chunk` 注入 `window.feedAudioPacket(...)`；HTML 用 Call 专用 WebAudio 时间轴按到达顺序播放，并对每个 `audio_id` 回发带 `stream_type=call` 的完成/停止消息。
...
=======
`CallAudioController` 不复用聊天 `MessageProcessor`：

```ts
class CallAudioController {
  append(packet: CallAudioChunkPayload): void;
  markResponseCancelled(responseId: string): Promise<void>;
  onWebViewFinished(audioId: string): void;
  onWebViewStopped(audioId: string): void;
...
1. 按服务端 `seq` 校验分片；重复分片丢弃，缺包记录错误并停止该句。
2. 按 `audio_id` 聚合，按首次出现顺序排队。
3. 同一时刻只把一个 `audio_id` 注入 WebView；下一句可以在 JS 层缓冲，但不能提前播放。
4. `audio_finished` 后发送 `call.playback_completed`，再开始下一句。
5. 收到 `call.stop_playback(response_id)` 后，删除该 response 的待播句，向 WebView 注入 `stopAudioPlayback(currentAudioId)`。
6. 收到 `audio_stopped` 后发送 `call.playback_stopped`。ACK 表示播放器确实停止，而不是仅表示已收到服务端命令。
7. 已取消 response 的迟到包一律丢弃。

### 5.8 `/call_ws` Transport

`CallWebSocketTransport` 与聊天 transport 分离，避免两个状态机互相污染：

```ts
class CallWebSocketTransport {
  start(): void;
  stop(): void;
  startCall(clientMsgId: string): Promise<AckResult>;
  resumeCall(callId: string): Promise<AckResult>;
  appendAudio(callId: string, seq: number, audio: string): void;
  hangup(callId: string): Promise<AckResult>;
  playbackCompleted(ids: PlaybackIds): Promise<AckResult>;
  playbackStopped(ids: PlaybackIds & { reason: string }): Promise<AckResult>;
>>>>>>> dev
}
```
...
=======
- 心跳周期：2 秒；服务端 5 秒无心跳视为连接丢失。
- 首次断开后立即重连，后续间隔 250 ms、500 ms、1000 ms，不能使用聊天 transport 最长 30 秒的退避。
- 重连总时限以服务端 `reconnect_deadline` 为准，本地上限 5 秒。
- 发送队列最多保留 25 个音频帧（约 1 秒）。超过时丢弃最旧帧并记录错误；进入 `RECONNECTING` 时清空，不跨断线恢复发送。
- 单条 JSON 消息最大 32 KiB；40 ms PCM 帧远低于该限制。
>>>>>>> dev
...
=======
| 电话行为规则 | `res/agent/prompts/call_system_prompt.jinja2` |
>>>>>>> dev
...
<<<<<<< HEAD
7. Release APK 中包含 `audio_stream_player.js`，且 Live2D HTML 包含 `stream_type`、`audio_id` 和 `stopAudioStream` 接口，全屏模型、表情和音频均可用。
=======
