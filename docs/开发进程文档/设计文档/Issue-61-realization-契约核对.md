...
第 1—3 节保留初稿的代码证据、风险与建议；第 4 节记录后续会话已经确认的结论。当前以 [realization SPEC](../../项目说明/项目架构与接口（spec）/接口文档/domain/realization.md) 为准，尚未实现。
...
- R1：新增 StartThinking，作为本请求首个独立 ActionPlan 发出。stage 在接收后直接消费并发送 thinking；结束、异常和取消由 stage 按请求归属清理。通知不进入语音执行队列，不调用 realize，也不等待 ExecutionReport。已接收通知仍记录在 HandlingReport 的计划 ID 中。
- ExecutionContext：由 stage 在业务计划出队时创建，传入 realize_action_plan，提供执行身份、执行时的交互身份/修订和独立取消令牌；开始思考通知不使用它。
- 接收结果：ACCEPTED/ALREADY_ACCEPTED 都是成功状态；发生明确拒绝时抛出 SinkRejectedError。初稿“明确拒绝使用”是措辞歧义，已经改正。
- R3：以 MessageEndOutput 替代 AudioEndOutput，目标 kind 使用 MESSAGE_END。它表示一条消息结束，纯文字也必须有终止输出；TextFinalOutput 只表示文本定稿。结束状态承载正常、失败或取消，映射现有终止包及音频错误字段。
- R6：按用户确认的现行客户端行为，保留“音频/表情 → 消息终止包 → normal 表情包”顺序；客户端在收到终止包后等待播放完再处理后续包。本版不新增播放完成回执。
- 顺序范围：本版依次执行业务计划及 Action、依次发送 AgentOutput；严格乱序检测、丢包恢复和跨连接重投去重不作为本版交付要求。成功重复接收状态只表达接收器确实识别出的事实。
- 其余初稿中的类型精简与暂缓范围保留，状态更新为本轮已确认目标契约；没有修改代码或远程工单，也不将静态文档视为运行时兼容验收。
