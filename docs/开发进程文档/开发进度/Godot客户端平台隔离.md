...
- 冷检查隔离 APPDATA 后，临时导出首次因导出模板位于主机用户目录而拒绝；未将该环境错误计为通过。`Invoke-GodotChecked` 增加显式主机用户数据开关，仅导出阶段使用，普通测试仍保持一次性 APPDATA/LOCALAPPDATA 隔离。
- 临时 Windows 导出、独立启动和版本输出验证通过（`agentluo-0.1.3`，未打包、未发布）；提交 `7bda250`。导出临时目录位于仓库外，未纳入版本控制。
