...
- 总体状态：已按最新授权收缩为独立提取，已提交至功能分支 `refactor/vcpedia-wikitext`，未合并。以下旧 scope/缓存/批次/补偿及来源判卷记录仅为历史事实；扩大方案已退役，不是现行契约。与兼容迁移进度分开记录。

## 已完成

### 2026-09-09 Ruby 缺口跟随选用分支
...
### 2026-09-09 TextHover 可见正文与图片排除
...
### 2026-09-09 zh-cn 正文转换与 nowiki 保留
...
### 2026-09-09 独立审查阻断：普通容器歌曲框与 staff
...
### 2026-09-11 独立审查阻断修复：HTML Ruby 隐式正文
...
### 2026-09-09 独立审查 P1：不完整刷新不得覆盖完整知识
...
### 2026-09-10 提取提示词正向步骤与完整 JSON 示例
...
### 2026-09-12 冗余清理：派生规则、渲染材料、oracle、配置双写
...
- 本轮执行约束：不联网（除全量 `tests` 中既有联网用例）、不调用真实模型、不 commit/push。作者自审完成，保留 HEAD `96c8726` 与全部其他未提交工作；本切片当时未单独提交，SPEC/Red/Green 的阶段门禁以工作区状态呈现，后随 `68d8a5f` 提交收录。

### 2026-09-13 提交前复审文档一致性修正

- 无产品行为变化。提取契约修正三处与实现冲突的旧句：补提新字段按材料同一次转换的结果作为最终文本合入、不做第二次转换（新摘要沿已有保护转换一次）；已标记不完整的请求字段可被替换、完整且未请求的字段不覆盖；材料为转换并剔除计数统计后的源码文本；同步移除已删除的 `rendered=True` 表述。
- 本进度文档与迁移进度文档总体状态更新为"已提交未合并"，移除全部"（本地未提交）"标题注记；此前条目中"无 commit/本地未提交"为撰写时事实，现已由 `b6a9e48`（SPEC/PRD/进度冻结）、`d34089b`（提取管道与模板规则配置）、`68d8a5f`（材料剔除计数统计）及本修正提交收录。
- 清理 `test_vcpedia_fixed_acceptance.py` 与 `test_vcpedia_sync_outputs.py` 文件末尾多余空行，`git diff dev --check` 恢复退出 0。
