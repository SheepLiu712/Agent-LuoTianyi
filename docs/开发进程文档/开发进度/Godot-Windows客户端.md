...
### 2026-09-20 界面场景化第七片——chat_view 与 image_overlay 场景化
- 交付行为：聊天视图与图片浮层改为「场景节点树 + 只做行为的脚本」。新增 res://scenes/ui/chat_view.tscn（根 MarginContainer「ChatView」，挂 src/ui/chat_view.gd 与 app_theme.tres）：原 _draw() 的整幅 #f5f8fb 改由根下 Background（ColorRect、忽略鼠标）承担，22 像素四边内边距下移到 %Margin，Column 的子节点顺序与改造前的 add_child 顺序逐项一致（Heading → HSeparator → HistoryRow → %Scroll → %Empty → %Latest → %Unread → HSeparator → AudioControls → %Input → Footer），%ClearDialog 挂在根下；Heading 内的 42×42 圆形头像（round_avatar.gdshader 的 ShaderMaterial + tianyi_icon.png）、「和天依聊聊」22 号、%Status 13 号 #607f8d、%Dynamics「动态」与 %ChatMore（unified_dropdown.tscn 实例）全部写进场景，%ChatMore 的 action_menu、「更多 ···」与 set_items 仍由脚本在注入后设置；%Input 是带 src/preview/composer_input.gd 的 TextEdit，%Scroll 仍是 virtual_message_list.tscn 实例。新增 res://scenes/preview/image_overlay.tscn（根 PanelContainer「ImageOverlay」）：panel 样式盒（#f7fafc、圆角 16、内边距 20）、Bar 内的 %Title 19 号 #344c59 与「－」「＋」「关闭」、%Scroll/%Picture、「发送图片 · 离线演示」的 %Confirm 与 %Feedback（后两者常驻但默认不可见，pending 为真时显示）。两个脚本删除带参 _init、chat_view 的 _draw()、Style 常量与全部建树代码，改 setup() 注入与 @onready + %Name 绑定；依赖为空时 _ready() 直接返回，因此无依赖实例化仍保留完整节点树。消费点 src/application.gd（const ChatView 改指场景，instantiate() as Control + setup(_chat)）与 src/preview/chat_preview.gd（ImageOverlay 常量改指场景，instantiate() + setup(texture,pending)）同步改造；浮层父节点由视图根改为 %Margin，几何与改造前逐像素一致。公开信号与全部公开方法、文案、尺寸、颜色、字号逐字未变。
- interface spec：接口文档 client_godot/README.md「chat_view 与 image_overlay 场景化（0.1.2 第七片）」。实现与契约一致；补充一条实现事实：契约守卫在不注入依赖时实例化场景，因此 %Close 的「取消/关闭」、%Title 的「待发送图片/图片预览」与 %Confirm 的按下回调都只在注入后设置，场景里存的是未注入态的默认值。
- commit：SPEC 37b024e，Red 95cfc79，Green 4adcae6。
- Red 证据：两个目标场景尚不存在时的真实失败（实测，逐字抄录）：test_ui_scenes.gd 报「FAIL: scene exists: res://scenes/ui/chat_view.tscn」「FAIL: scene exists: res://scenes/preview/image_overlay.tscn」并以退出码 1 结束；test_live_chat.gd、test_voice_chat.gd（run_websocket_tests.py --script）与 test_history_media.gd（run_history_tests.py --script）均报「FAIL: chat view scene exists」并以退出码 1 结束；capture_voice_ui.gd 的 VIEW_SCENE 门禁同样指向缺失场景。产品实现未动，未伪造 Red。
- 验证及结果：Godot 4.7.1 headless test_ui_scenes.gd PASS（新增两条契约：场景存在性、根节点名与类型、根脚本、setup() 暴露且 _init 无参数、%Margin/%Dynamics/%ChatMore/%Status/%HistoryStatus/%HistoryRetry/%HistorySkip/%Scroll/%Empty/%Latest/%Unread/%Volume/%StopVoice/%Input/%Send/%ClearDialog 与 %Title/%ZoomOut/%ZoomIn/%Close/%Scroll/%Picture/%Feedback/%Confirm 的 unique_name_in_owner 与 %Name 解析、从代码搬入场景的属性（Background 的颜色与忽略鼠标、两处 22 像素内边距、间距 12、头像 42×42 与展开/拉伸/纵向对齐、标题与状态的字号颜色、历史行的字号与智能换行、滚动区纵向扩展与横向滚动关闭、滑块的 0–1/步长 .01/最小宽 110/tooltip、输入框最小高 92 与占位与 LINE_WRAPPING_BOUNDARY、发送按钮的 96 宽与 PrimaryButton 变体、确认框的四个文案）、image_overlay 的 panel 样式盒、两处根带 app_theme）；scripts/check.ps1 21 步全绿；check_accounts.ps1、check_features.ps1、check_network.ps1 全部 PASS（含 live-chat 的单输入框与 ChatMore 菜单、voice-chat 的滑块与「清理语音缓存」确认框、history-media 的「打开原图」→「图片预览」、application-window 未登录时 TextEdit 数为 0、application-drafts 经 ChatMore 打开日志/相处模式/模型三个窗口）。
- 视觉：真机 GPU 截图 capture_voice_ui.gd 与 capture_release_ui.gd 产出 12 张 agentluo-011-*.png，与改动前基线（6a2e7c9 独立 git worktree、同机同命令重跑）比对：login、models、preferences、dynamics、dynamics-scale-125、dynamics-scale-150 六张 SHA256 完全相同；logs 与五张 voice-ui 改用像素级比对——logs 的差异是含时间戳的既有不稳定性（本片与基线差 3300 个像素、差异带 137,29–795,519，而基线自身两次运行差 3911 个像素、差异带位置相同 137,29–795,536）；五张 voice-ui 的聊天区（1200×800、paused、scale-125、scale-150 以 540×缩放因子为界，960×640 以 432 为界）在「基线自比对」与「基线与本片」两种比对下都是零差异像素，差异只出现在会动的 Live2D 头像区（左半屏，如 1200×800 基线自比对 10863 个差异像素、本片与基线 6099 个），人工查看 1200×800 与 paused 两张：标题与「已连接」、动态与「更多 ···」、气泡、语音行的「重放」与波形与秒数、「语音音量」滑块与「停止语音」、输入框与「Enter 发送 · Shift + Enter 换行」「发送」的字号、颜色、位置均与基线一致。capture_dropdown_ui.gd 与 capture_dynamics_ui.gd 未纳入本片比对（两者不加载 chat_view）。
- 作者自审：diff 只含两个新增场景、两个既有脚本的行为收敛与两个消费点的改造；用公开面清单比对逐项确认信号与方法未增未删未改名（移除的只有 Style/Composer/Dropdown 三个常量、建树用的局部 var、带参 _init 与 chat_view 的 _draw()），字符串比对确认除三个颜色字面量（#607f8d、#94a5af、#f7fafc）改为场景里的 float 值以外没有文案变化，且这三个颜色由契约用例按 Color("...") 断言相等；两个 addons DLL 的既存删除未纳入提交；无顺手改进、无其它视图改动。
- 未验证范围：未做人工多 DPI 复核；image_overlay 的浮层外观没有 GPU 截图（只有 history-media 的「打开原图」→「图片预览」断言与契约守卫）；未推送、未开远程 PR。

### 2026-09-20 界面场景化第八片——动态窗口与详情面板场景化

- 交付行为：动态窗口、动态详情面板与两种动态行改为「场景节点树 + 只做行为的脚本」。新增 res://scenes/ui/dynamics_window.tscn（根 Window「DynamicsWindow」）、dynamic_detail.tscn（根 ScrollContainer「DynamicDetail」）、dynamics_post_row.tscn（根 Button「DynamicsPostRow」）与 dynamic_comment_row.tscn，四个场景根均挂 app_theme.tres；窗口面板、工具栏（%Unread / %Publish / %Refresh / %ReadAll）、%Notice、%Split 的左右分栏（%ListScroll / %List、%More、%Right / %Empty）、行内四边内缩与 %Avatar 圆形遮罩、%Author / %Meta / %Excerpt 的省略号截断、评论行结构与详情面板 %Column 的子节点顺序（%Avatar / %Author / %CreatedAt / %Body / %Notice / %CommentDraft / %Send / %Comments / %ReplyBox / %ReplyLabel / %ReplyDraft / %CancelReply / %ReplySend / %Status / %LoadMore）全部写在场景里；行组件新增 src/ui/dynamics_post_row.gd 与 src/ui/dynamic_comment_row.gd，原 _ignore_mouse() 的 mouse_filter=IGNORE 改由场景承担、TYPES 映射搬进行脚本。dynamics_window.gd 与 dynamic_detail.gd 删除带参 _init 与建树代码，改 setup(controller, layout_path) / setup(controller, post) 注入并用 %Name 绑定；分栏比例夹取、_reply_box.reparent、行增量建 / 排序与选中态样式仍留在代码里。消费点 src/application.gd 改为 preload 场景 + instantiate() as Window + setup() + add_child()。公开信号与公开方法、文案、尺寸、颜色、字号逐字未变。

- interface spec：接口文档 client_godot/README.md「动态窗口与详情面板场景化（0.1.2 第八片）」。

- commit：SPEC bbd4d91，Red 8d2a279，Green c17246e。

- Red 证据：四个目标场景尚不存在时的真实失败（实测，逐字抄录）：test_ui_scenes.gd 报「FAIL: scene exists: res://scenes/ui/dynamics_window.tscn」「FAIL: scene exists: res://scenes/ui/dynamics_post_row.tscn」「FAIL: scene exists: res://scenes/ui/dynamic_comment_row.tscn」「FAIL: scene exists: res://scenes/ui/dynamic_detail.tscn」并以「UI scenes: FAIL」与退出码 1 结束；run_dynamics_tests.py --script res://tests/test_dynamics_window.gd 报「FAIL: dynamics window scene exists」并以「RuntimeError: dynamics test failed」与退出码 1 结束，test_dynamics_detail.gd 同。产品实现未动，未伪造 Red。

- 验证及结果：Godot 4.7.1 headless test_ui_scenes.gd PASS（新增四条契约：场景存在性、根节点名与类型、根脚本、setup() 暴露且 _init 无参数、上述关键节点的 unique_name_in_owner 与 %Name 解析、从代码搬入场景的属性、字号、颜色与样式盒）；scripts/check.ps1 21 步全绿；check_features.ps1（含 Application drafts）、check_accounts.ps1、check_network.ps1 全部 PASS（含 application-window 未登录时 TextEdit 数为 0、dynamics-detail 的分栏子节点顺序）；run_dynamics_tests.py 的 test_dynamics_window.gd 与 test_dynamics_detail.gd PASS。

- 视觉：真机 GPU 截图 capture_dynamics_ui.gd 产出 7 张 agentluo-011-dynamics-*.png（list / detail / reply / publish / minimum / scale-125 / scale-150），与改动前基线（82f6e05 独立 git worktree、同机同命令重跑）7 张 SHA256 全部相同；capture_release_ui.gd 的 7 张中 login / models / preferences / dynamics / dynamics-scale-125 / dynamics-scale-150 六张 SHA256 相同，logs 一张有差异但属既有不稳定性（基线自比对同样 DIFF，5185 个像素 vs 本片 3531 个，差异带位置相同 137,29-795,536，来源是含时间戳的运行标题与日志行）。人工查看 dynamics 与 dynamics-detail 两张：列表选中态蓝底与左侧蓝边、圆形头像遮罩、行内三行文案、评论「回复 测试用户：合成评论」前缀、时间与「回复」按钮、输入框与发送按钮均与基线一致。

- 作者自审：diff 只含四个新增场景、两个新增行组件脚本与其 .uid、两个既有脚本的行为收敛与一个消费点的改造；用公开面清单比对逐项确认信号与公开方法未增未删未改名（移除的只有 Style 与 TYPES 常量、建树用的局部 var、带参 _init 与被场景承担的 _ignore_mouse()），除颜色字面量改为场景里的 float 值以外没有文案变化，且这些颜色由契约用例按 Color 构造断言相等；两个 addons DLL 的既存删除未纳入提交；无顺手改进、无其它视图改动。

- 未验证范围：未做人工多 DPI 复核；未推送、未开远程 PR。


### 2026-09-20 界面场景化第九片——头像面板与预览

- 交付行为：头像背景、遮罩、驱动节点、错误提示和重置按钮改由 avatar_panel.tscn 提供；avatar_preview.tscn 继承该场景；正式应用和离线样板均实例化角色场景，移除旧头像预览路径。
- interface spec：客户端接口「头像面板与预览场景化」。SPEC 3cf2256；Red 0a29328；Green 为本记录所在提交。
- 验证：在独立复制工程（独立用户目录、Godot 4.7.1）运行 check.ps1（21 步）、check_accounts.ps1、check_network.ps1、check_features.ps1，全部通过。首次冷导入生成纹理缓存后重跑导入通过；原工作区编辑器占用 DLL，未关闭用户编辑器。
- 视觉：capture_voice_ui.gd --gpu 的 1200×800、960×640、暂停、125%/150%内容缩放五组截图及布局检查通过；查看 1200×800 角色与聊天布局正常。本次不声称重新完成与历史基线的像素比对。
- 作者自审：已对照原脚本检查节点顺序、加载失败隐藏重置按钮、场景继承、消费点及实际 diff；保留原有交互接口。
- 未验证范围：公共服务器、人工听感、操作系统 DPI 切换、安装包；未推送或创建远程 PR。

### 2026-09-20 场景迁移回归修复：系统消息

- 交付行为：系统消息隐藏角色头像和空白气泡，仅显示居中提示文字；符合已有「消息气泡与虚拟列表场景化」契约，无新增 interface。
...
### 2026-09-20 场景化最终遗漏修正与验收

- 交付行为：偏好/模型的四个固定下拉直接内嵌到场景，保留唯一节点名；主场景组件错误提示常驻隐藏；菜单分隔项使用子场景；动态选中样式、下拉/投递/日志状态配色资源化。除既定草稿确认框与 Live2D 模型/特效例外，UI 产品脚本不再 new() 控件或 StyleBox。
- interface spec：客户端接口「场景化遗漏修正」；SPEC 118325b，Red b6bee28（7 项静态场景/主题契约真实失败），Green 为本记录所在提交。公开信号、业务方法与注入约定不变。
- 验证：隔离复制工程和独立用户目录运行 check.ps1 24 项、check_accounts.ps1、check_network.ps1、check_features.ps1 全部通过。后续补充状态配色条目与含分隔线的下拉导航回归后，test_theme_contract、test_dropdown、test_log_window 再次通过；新增配色断言是补回归，未伪造 Red。
- GPU：capture_release_ui 与 capture_dropdown_ui 通过。登录、模型、相处模式、动态及125%内容缩放截图与调整前逐像素相同；150%动态仅「发送评论」按钮的悬停/普通配色不同，已查看裁剪确认，不是布局变化。离线五场景聊天区零像素差异的证据见第十片记录。
- 文档：总体设计补充当前场景资源边界；架构索引明确标记 42b5b1c 系列为历史快照并链接现行契约，避免把旧薄场景/样式工厂当现状。
- 作者自审：逐项复核 SPEC、Red、实现 diff、公开场景结构、节点所有权、主题来源、旧引用和 git diff --check；本地用户编辑器保持运行，DLL 临时副本未纳入提交。
- 未验证范围：公共服务器、系统文件选择器人工操作、真实 OS DPI/IME、长期性能与安装导出包。仅本地提交，未推送或创建远程 PR。
