...
### 2.2 好感度系统已移除 — 减少 ~50ms/次 LLM 调用

**文件**: `server/src/agent/activity_maker.py`（已移除 `affection_ctx` 注入）
**严重度**: ✅ 已修复

好感度系统此前在每次对话时会额外调用 LLM 分析用户消息以计算好感度变化。该功能已从 pipeline 中移除：

1. `luotianyi_agent.py` 的 `generate_topic_reply_for_pipeline` 不再注入好感度上下文
2. `activity_maker.py` 不再在 `REGULAR_LOGIN` 流程中查询好感度
3. 好感度模块代码保留但不再在 pipeline 中注册

**性能收益**: 每次对话减少约 1 次 LLM API 调用（约 200-800ms 延迟，取决于模型）

### 2.3 Redis 乐观锁重试的三次硬编码循环
...
## 9. 本次更新（2026-05-15）的变更评估

### 9.1 好感度系统移除 - 性能改进

**变更**: 好感度系统已从服务端 pipeline 中移除（保留模块代码）。
**影响**: 正面性能改进

| 指标 | 之前 | 之后 | 改善 |
|------|------|------|------|
| 每轮对话 LLM API 调用次数 | 1（回复）+ 1（好感度分析）| 1 | 减少 50% |
| 每轮对话延迟 | ~1.5-3s（含好感度） | ~1-2s | 降低约 30% |

### 9.2 表达风格偏好注入优化

**变更**: 偏好设置作为独立 prompt 变量注入（`preference_context`），而非追加到 `user_description`。
**影响**: 正面性能改进

| 指标 | 之前 | 之后 | 改善 |
|------|------|------|------|
| prompt 结构清晰度 | 偏好混在用户描述中 | 独立段落 | 更好 |
| token 消耗（无偏好时） | 偏好部分仍占空间 | 完全省略 | 减少 10-30 tokens |
| token 消耗（有偏好时） | 拼接在描述后 | 独立模板页 | 无显著变化 |

### 9.3 日程系统重构 - 数据库变更

**变更**: 事件存储从 JSON 文件迁移到 SQL 数据库，新增 `Event` 和 `EventNotification` 表。
**影响**: 中性（基础设施改进，运行时无明显变化）

| 指标 | 之前 | 之后 | 改善 |
|------|------|------|------|
| 数据持久化 | JSON 文件全量读写 | SQLite 索引查询 | 更高效 |
| 并发写入 | 文件锁竞争 | WAL 模式并发 | 更安全 |
| 通知去重 | Python 层 JSON 字典 | SQL 层唯一索引 | 更可靠 |
| 启动时间 | 读取全量 JSON | 惰性数据库查询 | 更快 |
| 每轮对话数据库写入次数 | 2（对话 + 好感度日志）| 1 | 减少 50% |
| 每轮对话额外延迟 | 好感度 LLM 分析 ~500ms-2s | 0 | 消除 |
| 每日好感度更新检查 | 每次对话都检查 | 0 | 消除 |

**关键改进**: 好感度分析调用了额外的 LLM API（SiliconFlow/DashScope），移除后：
- 减少了 API 调用开销和网络延迟
- 减少了数据库写入操作（`AffectionLog` 表写入）
- 减少了 LLM token 消耗，间接降低成本
- 简化了 `add_conversation` 调用链

### 9.2 表达风格偏好注入方式改进

**变更**: 将偏好设置从 `user_description` 拼接改为独立的 prompt 段落。
**影响**: 轻微正面

- **之前**: `user_description = (user_description + "\n" + pref_context).strip()` — 偏好混入用户描述中，可能干扰 LLM 对用户画像的理解
- **之后**: 在 prompt 模板中增加 `preference_context` 独立段落，使用 Jinja2 条件渲染（仅在有设置时显示），LLM 能更准确地理解这是用户对角色互动方式的偏好，而非用户自身特征
- **性能影响**: 无显著变化（相同的信息量，只是组织方式不同）

### 9.3 新增接口与 UI - 性能与安全影响

**新增 `/auth/reset_account` 端点**:
- 性能影响：可忽略（简单数据库更新操作）
- 注意：需配合频次限制使用，防止被用于枚举邀请码

**新增自定义服务器地址功能**:
- 性能影响：仅影响启动时的 `/auth/public_key` 验证调用（一次 HTTP GET）
- 注意：如果用户频繁切换服务器地址，可能因连接重置导致短暂的 WebSocket 断连

### 9.4 未解决的性能问题

以下本次更新未涉及的性能问题（仍存在于代码库中）：

1. **全局 TTS Worker 单消费者** — 所有用户 TTS 串行处理（P0）
2. **全局 SQL 写锁串行化** — `SQLWriter` 的全局 `RLock`（P0）
3. **记忆写入前的多次向量去重搜索** — 每个记忆写入都触发 embedding API（P1）
4. **同一 pipeline 回合内重复读取 Redis context** — 3 次读取（P1）
5. **流式 TTS 同步生成器阻塞事件循环**（P2）
