...
AgentRuntime 创建每角色 router，并通过 Agent 的装配参数传入；装配参数只供运行时和模块内测试使用，不从门面暴露 router。生产装配为每个角色注册 SayHandler，共用 SpeakingSkill；刺激侧注册 InteractionEndingHandler。任一 router 构造失败时，AgentRuntime 初始化失败，沿用已有初始化清理规则，不发布半成品运行时。
...
刺激路由登记 `INTERACTION_ENDING -> InteractionEndingHandler`。处理器调用当前角色 ContextFactory.release 释放已有交互上下文，返回 HandlingReport，不产生行动计划。
