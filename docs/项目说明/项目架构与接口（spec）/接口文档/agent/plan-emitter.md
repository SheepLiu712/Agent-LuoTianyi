...
| 外部确认后本地确认写入失败 | 保留原计划；本次返回 DEPENDENCY_UNAVAILABLE 并保留内存中已确认 ID，不伪造持久成功，不另建计划 |

“是否仍待重投”与“最后接收结果为已确认、明确未新增或未知”是不同事实。一次未知结果之后再遇永久拒绝不能据此断言历史未接收。报告中的 emitted_plan_ids 只列已确认事实，不把未知说成已接收或未发生。永久拒绝不自动再次投递；恢复遇永久拒绝时终态保留原认知消费字段，错误码采用最后阻止投递的拒绝映射，retryable=False，保留所有真实确认 ID。
...
确认写入失败若之后可以保存 provisional，重建 Agent 可用原计划恢复；若 provisional 保存也失败则保留占用、拒绝接管。拥有者在 Handler 尚未结束或任务取消而未形成 provisional 时，即使 outbox 存在也不能自动重放，否则无法知道认知是否会继续产生效果。恢复调用计入运行时在途所有权，关闭不能提前释放数据库或遗留 sink 任务。
