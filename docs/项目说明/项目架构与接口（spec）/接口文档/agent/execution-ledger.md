...
拥有者 task 取消沿用门面受控清理与 CancelledError 传播规则，重复取消只向处理器转发一次。无可信结果的在执行行动保持未知；并发等待者取得依赖不可用报告，不接管任务。所有已进入账本的调用及等待者计入运行时在途集合；shutdown 停止接受后等待清理与结算，超时保留依赖供后续关闭重试。
