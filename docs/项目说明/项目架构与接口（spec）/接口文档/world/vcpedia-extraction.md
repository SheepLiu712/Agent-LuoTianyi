...
独立补提 prompt 只请求 needed 信息框字符串字典、summary 字符串数组、lyrics 原文字符串，不请求 short_summary；提供正向示例及材料不足填 null 示例。输入 song_data、needed、materials 均必填，materials.raw 保留原源码。响应只解析 JSON 对象与字段类型，坏 JSON/null/异常/超时保持规则值继续摘要；只合并请求的缺项。补提新字段及新摘要各沿已有保护转换一次，不覆盖已有字段，不做语义判卷。source_extraction 只收集材料与合并，不调用模型；必要嵌入先于模型，程序渲染后直接合并（`rendered=True`，保持站点字形），渲染文本不作为模型材料——材料只含 `raw` 源码，模型的价值在读源码而非重读已渲染制品——因而不改变其预算。
