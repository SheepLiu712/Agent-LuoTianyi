...
>
> 输入沿用 [handle 输入契约](handle-input.md)，处理结算沿用 [HandlingReport](handling-report.md)。
...
`WriteDiary` 已有生产 action handler：按角色发布私密（`PRIVATE`）、禁止评论、`source_type="diary"` 且业务去重身份为角色＋用户＋日期的日记动态，成功报告 `EffectRef(kind=DYNAMIC_POST, effect_id=<动态 id>)`；发布失败返回 `DEPENDENCY_UNAVAILABLE` 且不声称已提交效果。它只提交持久效果，不投递聊天输出。
