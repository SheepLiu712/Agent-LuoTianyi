# 计划与 realization 领域契约草案

> 状态：2026-09-06 第一版，待评审，尚未实现。
>
> 对应 [issue #61](https://github.com/SheepLiu712/Agent-LuoTianyi/issues/61)。本文中的新增、精简及暂缓项均为建议；确认前不作为 RED 的期望来源。
>
...
| 具体行动 | `Say`、`Sing`、`WriteDiary`、`PublishDynamic`、`ReplyDynamic`、`RequestSongLearning` |
...
| 输出 | `AgentOutput`、`TextFinalOutput`、`AudioChunkOutput`、`AudioEndOutput`、`ExpressionOutput`、`AudioFraming`、`AudioEndStatus`、`AudioErrorCode` |
...
构造错误码建议为 `CONTRACT_INVALID_ACTION`、`CONTRACT_INVALID_PLAN`、`CONTRACT_INVALID_EXECUTION_CONTEXT`、`CONTRACT_INVALID_OUTPUT`、`CONTRACT_INVALID_RECEIPT`、`CONTRACT_INVALID_EXECUTION_REPORT`、`CONTRACT_INVALID_VALUE`；协议值与枚举成员名相同。

## 3. Action 与值对象

`Action` 为不可直接构造的抽象基类，公共字段只有 `action_id: str`，具体类型提供固定 `kind: ActionKind`。
建议本轮 ActionKind 为 `SAY=say`、`SING=sing`、`WRITE_DIARY=write_diary`、`PUBLISH_DYNAMIC=publish_dynamic`、`REPLY_DYNAMIC=reply_dynamic`、`REQUEST_SONG_LEARNING=request_song_learning`。
...
演唱前后的说话分别用有序 Say 表达，草案不再提供位置不明确的 `Sing.bridge_text`。
...
source_stimulus_ids 内部唯一，保持传入顺序。引用范围建议为本次请求的触发刺激与 pending 的并集；主动到期刺激即使没有 pending 也可作为依据。
它表达计划依据，不代替 HandlingReport 的 considered/consumed/retained 结算。

构造时验证序号非负；从零连续的 ordinal、同一请求同一 ordinal 内容一致等跨计划关系由 plan sink 验证。
...
回执身份必须对应被提交计划；回执仅证明 sink 已可靠接收，不证明行动完成。

同一 request/ordinal 的同值计划重投返回 ALREADY_ACCEPTED，修改内容或复用身份提交另一计划时拒绝。
接收边界验证绑定的角色、交互、请求、可见刺激、当前修订和序号；接收后才能将计划记入 HandlingReport.emitted_plan_ids。
emit 可以等待以施加背压，但必须有界。接收过程不在当前调用栈中同步重入 realize。

明确拒绝使用 `SinkRejectedError`，公开只读 `code: SinkRejectionCode`。
建议代码为 `IDENTITY_MISMATCH`、`CONTENT_CONFLICT`、`OUT_OF_ORDER`、`STALE_INTERACTION`、`UNSUPPORTED_OUTPUT`、`SINK_CLOSED`、`BACKPRESSURE_TIMEOUT`，协议值与成员名相同。
明确拒绝表示本次值未新增接收；连接丢失导致结果未知时，使用原身份和原内容重投确认，不宣称没有接收。
...
AgentOutput 为不可直接构造的抽象基类。具体输出通过固定 kind 对应已有 AgentOutputKind，内容直接作为具体类型的字段，避免可任意搭配的 kind/content 二元组。
...
| `AudioEndOutput` | AUDIO_END | `status: AudioEndStatus`、`error_code: AudioErrorCode \| None` |
| `ExpressionOutput` | EXPRESSION | `expression: ChangeExpression` |

AudioFraming 建议为 `COMPLETE_FILE=complete_file` 和 `FILE_FRAGMENT=file_fragment`：前者每块是可独立解码的音频文件，后者需将同一 Action 的音频块依序拼成一个文件。依据是当前 TTS 逐块编码 WAV、演唱对整体音频切分字节，两种方式不能混为同一种无说明的 chunk。
媒体编码由文件头描述；本草案不重复传入文件头已有的采样率和声道，不引入原始 PCM。
预制媒体先由执行边界读取为 bytes；不再给 AudioChunkOutput 提供第二套 MediaRef 输出路径。

AudioEndStatus 为 `COMPLETED=completed`、`FAILED=failed`、`CANCELLED=cancelled`。
AudioErrorCode 为 `EMPTY_AUDIO`、`GENERATION_FAILED`，值同成员名。
FAILED 必须有错误码，其他状态为 None。没有产生任何音频也可以发送 FAILED 结束事件，保留已有显示文本。
这是建议新增的结束状态，需确认后才变更契约。

sequence_no 从零连续递增，以完整输出值检测重投冲突。它可同时排序文字、音频及表情；草案不再另设平行的 chunk_index。
每个 Say/Sing 对应一个展示消息、一条可选音频流；消息身份可由稳定 action_id 对应，持久化与 Adapter 必须使用同一映射。
收到 TextFinalOutput 表示文本定稿，不代表随后可选的音频结束；AudioEndOutput 表示生成/发送终止，不代表客户端播放完成。
绑定原计划的 sink 根据 Say 的音频来源或 Sing 类型判断是否等待音频终包；纯文字 Say 在文本定稿后完成消息。失败前尚未发出的显示文本仍应保留，避免音频错误使已决定的文本消失。

当前输出枚举中的 TEXT_DELTA、MOTION 已存在；本轮未找到相应生产行为，建议暂不增加具体输出构造类型。该建议不删除现有枚举成员。
...
sink 验证交互、执行和行动绑定，按 sequence_no 排序，对相同身份同值重投幂等接受，身份相同而内容不同时明确拒绝。
支持种类、当前失效状态、关闭和背压同样在接收边界验证；拒绝使用第 5 节的 SinkRejectedError。
...
EffectKind 建议只登记 `DYNAMIC_POST=dynamic_post`、`DYNAMIC_COMMENT=dynamic_comment`、`SONG_LEARNING_JOB=song_learning_job`，对应本轮实际效果。日记返回动态效果引用。

ExecutionErrorCode 建议为 `CONTRACT_MISMATCH`、`UNSUPPORTED_ACTION`、`UNSUPPORTED_OUTPUT`、`STALE_INTERACTION`、`SINK_CLOSED`、`BACKPRESSURE_TIMEOUT`、`DEPENDENCY_UNAVAILABLE`、`PROVIDER_TIMEOUT`、`AUDIO_EMPTY`、`AUDIO_GENERATION_FAILED`、`CANCELLED`、`INTERNAL_ERROR`，值同成员名。
...
5. `ExecutionReport.irreversible_effect_committed` 建议作为只读派生属性，等于各 ActionResult 对应值的 any，不接受重复构造参数。
...
| 纯文字回复、TTS 成功和无音频失败 | TextFinalOutput，加可选 AudioChunkOutput / AudioEndOutput |
| 唱歌标题、歌词、音频及“唱歌”表情 | Sing，经文字、音频、表情输出 |
| 登录预制欢迎且保存聊天记录 | Say + CONVERSATION |
| 触摸预制音频且不显示气泡、不落聊天记录 | Say + EPHEMERAL_REACTION |
| 音频为空、生成中断、取消后的客户端清理 | AudioEndOutput 的状态及错误；断线时本地通道清理 |
| 私密日记、普通动态和评论 | 对应行动的 owner、来源、可见性和评论策略 |

开始/结束思考建议归 ChatStage 的处理生命周期状态：进入实际内容决策时发送当前 `agent_state_changed(thinking)`，相关处理全部结束时发送 `waiting`；异常、取消和旧请求完成不得错误关闭新请求的思考状态。协调信号处理本身不自动表示思考。
它发生在计划产生前，零计划请求也可能需要该通知，故不建议以排队 Action 表示。精确起止边界及是否需要 Agent 的进度通知仍是待确认项，见核对记录 R1。

触摸恢复 normal 使用同一 Action 的 ExpressionOutput 表达。音频接收结束不能证明播放结束；播放后恢复的具体时序必须在客户端兼容边界另行确认，不能由 OutputReceipt 推断。

## 11. 契约验证入口

确认后从 `src.domain.agent` 的公开构造器验证合法值、字段缺失/多余、不可变性、Say 音频互斥、计划身份唯一、报告内部关系及错误分类。
两个 Protocol 的类型声明本身不证明排队、背压、重投或效果幂等；这些约束通过实际接收器和公开 Agent 调用验证。
