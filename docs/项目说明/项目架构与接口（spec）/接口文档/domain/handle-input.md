...
> 状态：SPEC 第一版，待评审；本文新增类型尚未实现，不代表 Agent handle 已可调用。
>
> 日期：2026-09-06
>
...
- `InteractionKind`、`ConnectionState`、`AgentOutputKind`、`SnapshotRef`。
- `CancellationToken`、`CancellationReason`。
- `InvalidHandleInputError`、`HandleInputErrorCode`。

除 `CancellationToken()` 外，数据对象采用仅限关键字的直接构造器。下文所有字段均须显式提供，包括可空字段；固定 `kind` 不作为构造参数。请求、快照、引用和集合不可变，不能通过保留外部可变集合改变已构造快照。集合必须按声明类型传入，不隐式转换 list、set 或 dict。
...
`pending_stimuli` 必须包含实际的已构造强类型 Stimulus，不能只提供 ID、旧 Mapping Stimulus、字典或 Agent 内部 `UnreadMessage`。Agent 只对本快照中的内容进行本轮判断，不能根据 ID 回查 stage 补齐内容。
...
| 字段 | 类型 | 含义与约束 |
| --- | --- | --- |
| `conversation_ref` | `SnapshotRef` | 已冻结的对话摘要和近期记录的受控引用 |
...
| `visible_world_ref` | `SnapshotRef` | 角色被允许读取的 world 事实快照引用 |
...
### 3.6 SnapshotRef 与 supported_outputs

`SnapshotRef(snapshot_id: str)` 为不可变名义类型，唯一字段 `snapshot_id` 非空白。它引用固定版本的快照，不是“读取最新状态”的可变别名。构造器不读取资源、不做授权；引用的存在性、版本与访问权限由消费引用的受控 port 校验。`MediaRef`、`EvidenceRef` 等其他引用不能代替 `SnapshotRef`。本版不新增引用解析 port。
...
| 成员及值 | 使用场景 |
| --- | --- |
| `CONTRACT_INVALID_INTERACTION` | snapshot/引用自身非法，例如负 revision、无时区时间、重复 pending ID、协调信号混入 pending、活动 ID/revision 不配对 |
...
| 负 revision、bool revision、无时区时间、错误引用类型 | 对应稳定错误；合法的过去 deadline 可保留 |
...
以下场景需要未来真实 Agent/stage 公开入口验证，不能用领域对象测试宣称通过：预取消不启动工作；Recall/模型迟到结果不提交；新 revision 拒绝旧计划；同 request ID 的输入冲突；断线和重连期间的 pending 结算；引用授权及版本读取。

本次为文档交付，运行时 Red/Green 不适用。评审验证采用字段逐项核对、Markdown 相对链接检查及 `git diff --check`；未建立新类型或运行时行为测试。

## 8. 阅读验收

评审者应能仅根据本文说明：新消息到来与连接断开时分别由谁取消、使用哪个原因；旧 snapshot 是否变化；重新处理时三个 ID/revision 如何变化；取消前已接收的计划是否会自动回滚。不能从本文直接确定的 Agent 输出及结算细节，应从对应输出契约评审，不从内部代码推断输入语义。
