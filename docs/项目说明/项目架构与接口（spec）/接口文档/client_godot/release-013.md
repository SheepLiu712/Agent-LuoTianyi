...
关系与表达风格的当前值只在只读值区域显示一次，右侧40px预设箭头仍提供原稳定ID选项、选中提示及键盘操作。UnifiedDropdown新增场景属性show_selected_label（默认true，兼容已有调用）：false时保留场景预置的图标/文案，不把选中项再复制到按钮；不影响选中ID、菜单高亮或activated信号。两个预设场景使用false，并提供对应tooltip；仅下拉选择预设，其他偏好保存流程不变。

后续用户要求移除这两个菜单的“自定义”项：关系只列朋友/知己/偶像/搭档/家人，表达风格只列活泼可爱/温柔可人/文静恬淡。用户进一步明确这两项只能选择预设，不允许键盘输入、粘贴或IME编辑；性格关键词和补充上下文保持可编辑。已保存的非预设值可显示以避免静默改写，之后只能选择预设替换；空值或非预设值不错误勾选其它选项。UnifiedDropdown.clear_selection()清除选中ID、更新标题且不发activated；下次键盘向下从第一项开始，不影响既有set_selected_id的有效ID校验。

设置首次加载时相处表单不由灰底切为白底：只读与可编辑输入共用白色实底、同样的边框和文字色，预设箭头/重新加载的禁用外观保持原底色。读取、保存、错误状态仍明确显示，加载/保存期间和加载失败后仍禁止编辑；不提前启用表单或隐藏网络失败。此样式限定相处页，不改变其它控件的禁用语义。
