...
设置的相处模式/模型配置/语音缓存页在源码与导出程序中均铺满内容区域，初始隐藏再切换不得丢失尺寸；离线或读取失败也显示对应标题、说明和重试/未知容量反馈。退出登录按钮与分类侧栏同左边缘、同宽、居中文字，与右侧底部操作同一行。验证直接运行导出EXE加载外部验收脚本，检查实际可见内容区域，不能只依赖源码截图。
