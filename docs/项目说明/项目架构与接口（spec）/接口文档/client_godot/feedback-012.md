...
退出草稿确认依附当前触发窗口：设置退出登录显示于设置窗口，主窗系统关闭显示于主窗口；共用一个ExitDialog。取消/确认后回收至Application，关闭设置或重新登录后仍可再次使用，不随业务窗口销毁。保存中请求仍等待结果，失败后在仍存在的设置窗口展示草稿确认。GPU验证须从已聚焦设置入口触发，并确认对话框处于该窗口、取消默认聚焦及按钮可达，不能仅断言visible。
