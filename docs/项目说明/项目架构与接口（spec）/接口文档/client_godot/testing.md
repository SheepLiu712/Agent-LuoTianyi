...
当前入口以[测试README](../../../../../client_godot/tests/README.md)为准，旧预计脚本数量仅是当时整理目标。新增静态架构守卫归账户检查，缺失能力归基础组，未来扩展契约归功能组；保持原参数/环境变量及领域夹具独立。UI平台能力通过场景Resource注入；音频测试显式装配真实解码工厂，缺失工厂另测明确失败。历史迁移快照不再约束当前内部路径。
