...
本文保留平台隔离前的迁移记录，不作为当前窗口、装配或平台接口约定；现行入口见[接口索引](README.md)。原文如下。

# Godot 客户端 interface

0.1.3登录、账户本地状态、气泡与图片尺寸以[0.1.3契约](release-013.md)为准，替代下文相冲突的旧登录与定宽呈现条目。
...
视图场景按模块分目录：`res://scenes/ui/`（窗口与视图组件）、`res://scenes/preview/`（浮层等样板子视图）、`res://scenes/avatar/`（角色面板与冒烟场景）；`res://scenes/` 根只保留项目入口 `main.tscn` 与尚未一并迁移的离线样板 `chat_preview.tscn`。场景是提交里的文本 `.tscn`，不依赖在编辑器里手工搭建。
...
`executor` 注入为 `null` 时（未接入执行器的路径），`_ready()` 把 `%TestButton` 置为不可见、把 `_test_button` 与 `_test_dialog` 成员置空，并且不连接任何测试信号；注入非空时二者参与布局，并连接「先固化草稿再弹确认框」与 `confirmed` 两个回调。`Container` 布局忽略不可见的 `Control`，`%TestDialog` 也保持 `AcceptDialog` 默认的不可见状态，因此渲染结果与改造前「两个节点根本未被创建」一致；`tests/capture_release_ui.gd` 的模型设置截图与改造前逐字节相同。

两个窗口场景都允许不带依赖实例化，这是契约守卫 `tests/test_ui_scenes.gd` 与场景检查的前提：`PreferencesWindow` 在 `_controller` 为空时只完成基类初始化便返回，`ModelWindow` 仍按 `executor` 是否注入决定 `%TestButton` 的可见性、但在 `_settings` 为空时同样返回；两场景在无依赖时都不连接任何依赖信号，场景自身提供的属性仍然成立。生产路径固定按 `instantiate() → setup() → add_child()` 调用，因此 `_ready()` 执行时依赖总是已经注入。
...
从哪个 interface 验证：`tests/test_ui_scenes.gd` 按同一张表断言两个场景的存在性、根节点名与类型、根脚本、关键节点 `unique_name_in_owner` 与 `%Name` 解析、`setup()` 暴露且 `_init` 无参数，以及从代码搬入场景的属性、字号、字体颜色覆盖与样式盒取值（两个窗口的「无依赖实例化」也因此被这条用例覆盖）；`tests/test_preferences.gd`、`tests/test_model_settings.gd`、`tests/test_application_drafts.gd` 从窗口的公开行为（脏草稿关窗确认、草稿参与关闭守卫、菜单打开独立窗口）回归。
### account_view 与 main.tscn 就地节点化（0.1.2 第四片）

`res://scenes/ui/account_view.tscn`：根 `PanelContainer`（名 `AccountForm`）挂 `src/ui/account_view.gd`，依赖由 `setup(session)` 注入。场景提供 `Column`（`VBoxContainer`，间距 14）内的 `Title`「和天依再见面」（25 号字、`#344c59`）与 `Subtitle`「登录你的账户，继续这段陪伴。」（13 号字、`#809ba7`），`%Form`（间距 10）内的 `%AccountMode`（`UnifiedDropdown`）、五组输入框 `%Server`/`%Username`/`%Password`/`%Confirm`/`%Invite`（占位与 tooltip 文案含服务器地址示例、`secret`、纵向最小高度 40、`normal` 样式盒 `#f0f5f7`／圆角 8／内边距 10）、`%Remember`「下次自动登录」、`%Submit`「登录」（`PrimaryButton` 变体、纵向最小高度 42），以及列上的 `%Cancel`「取消请求」、`%Identity`、`%Logout`「退出登录」（三者默认不可见）、`%Status`（智能按词换行、13 号字、`#607f8d`）与 `%Logs`「打开日志」。原 `_ready()` 的 `theme = Style.make_theme()` 与 `add_theme_stylebox_override("panel", Style.box(Color("ffffff"),18,28))` 改由场景根的主题引用与 `panel` 样式盒承担；`AccountMode` 由代码 `name =` 命名改为场景里的 `unique_name_in_owner` + `%Name` 绑定。`_apply_mode()`、`_send()`、`_update_state()`、`_clear_secrets()`、`_error_text()` 与全部文案、尺寸、颜色行为不变，`_fields` 字典仍以 `server/username/password/confirm/invite` 为键。

`account_view` 的注入时机契约（场景内嵌实例必需）：`setup(session)` 只保存依赖、不建节点、不发起 IO，依赖到位后由私有 `_initialize()` 完成一次性接线（下拉项与 `activated`、五个输入框的 `text_submitted`、`%Remember`、`%Submit`、`%Cancel`→`_session.cancel`、`%Logout`、`%Logs`→`log_requested`、`_session.changed` 与 `_apply_mode()`）。`_ready()` 只在依赖已注入时调用 `_initialize()`，`setup()` 只在节点已就绪（`is_node_ready()`）时调用 `_initialize()`：独立实例化（`instantiate() → setup() → add_child()`，`_ready()` 时依赖已就位）与 `main.tscn` 内嵌实例（子节点 `_ready()` 早于父节点，依赖在父节点 `_ready()` 中注入）都恰好初始化一次。依赖为空时 `_ready()` 直接返回、场景节点树原样保留，这是契约守卫与场景检查的前提。

`res://scenes/main.tscn` 就地节点化为组合根：根 `Control`（名 `AgentLuo`）仍挂 `src/application.gd`，原 `_ready()` 用 `new()` 搭的骨架改由场景提供——`%Split`（`HSplitContainer`，全幅锚点、`dragger_visibility = DRAGGER_HIDDEN_COLLAPSED`、默认 `visible = false`）、其唯一子节点 `%Center`（`CenterContainer`，`custom_minimum_size.x = 440`、横向扩展填充）、`%Center` 下的 `%AccountForm`（`account_view.tscn` 内嵌实例，`custom_minimum_size.x = 390`）、`%LogProblem`（`Label`，顶边通栏锚点、智能按词换行、`#b72a2a`）与 `%ExitDialog`（`ConfirmationDialog`，标题「放弃未保存的内容？」、正文「设置或动态窗口中有未保存的内容，确认放弃并继续？」、「放弃并继续」/「取消」按钮文案）。`avatar` 与 `chat_view` 仍按登录状态在运行时实例化；角色栏仍以 `_split.move_child(_avatar, 0)` 插到 `%Split` 第 0 位。

`application` 的依赖注入改为 `setup(account_session, layout_path)`，删除带参 `_init`（`_layout_path` 默认值与 `setup()` 的默认参数都保持 `user://window_layout.cfg`）；`_split`/`_center`/`_log_problem`/`_exit_dialog`/`_account_form` 改为 `@onready` + `%Name` 绑定，`_ratio`、`_layout_ready`、`_expanded`、`_expanded_size`、`_exit_action` 仍是脚本状态。场景里的骨架默认不可见，正式路径在原来 `add_child(_split)` 的位置改为 `_split.show()`：预览（`--preview`）与「凭据保护组件缺失」这两条原本根本不创建骨架的路径因此不会多出控件。预览分支仍移除 `%LogProblem` 与 `%ExitDialog`，由 `free()` 改为 `queue_free()`（两者在预览里都不可见，渲染结果不变）。

`UnifiedDropdown` 仍未节点化：本片只把下拉节点放进场景以获得稳定的 `AccountMode` 名与位置，其 `custom_minimum_size`（140×40）、`alignment`、`clip_text` 仍由其 `_init()` 提供，第五片统一收敛。

从哪个 interface 验证：`tests/test_ui_scenes.gd` 增加 `account_view.tscn` 与 `main.tscn` 两条契约（根节点名/类型/根脚本、`setup()` 暴露且 `_init` 无参数、关键节点 `unique_name_in_owner` 与 `%Name` 解析、从代码搬入场景的属性与样式盒；`main.tscn` 条目以临时布局目录注入后实例化，应用自建会话的路径因此在守卫之内，临时目录在用例结束时清理）；`tests/test_account_view.gd` 改用 `load("res://scenes/ui/account_view.tscn").instantiate()` + `setup(session)`，`tests/test_application_window.gd`、`tests/test_log_window.gd`、`tests/test_application_drafts.gd`、`tests/capture_release_ui.gd` 改用 `load("res://scenes/main.tscn").instantiate()` + `setup(session, layout_path)`，从公开行为（登录失败保留草稿、注册成功回登录、退出登录恢复表单、日志窗口、草稿退出确认、音量与窗口尺寸）回归。
### 日志窗口与下拉控件场景化（0.1.2 第五片）

`res://scenes/ui/unified_dropdown.tscn`：根 `Button`（名 `UnifiedDropdown`）挂 `src/ui/unified_dropdown.gd` 与 `app_theme.tres`，场景提供选择器默认形态（`custom_minimum_size = (140,40)`、左对齐、`clip_text`）与私有弹层节点树 `%Menu`（`PopupPanel`，默认不可见、`force_native`、白底圆角 10 内边距 6、1 像素 `#dce1e5` 边框、`shadow_size 4`、12% 黑阴影）、`%Menu/%Scroll`（`ScrollContainer`，禁用横向滚动）、`%Menu/%Scroll/%Rows`（`VBoxContainer`，间距 0、横向扩展填充）。原 `_init(actions)` 删除：动作菜单模式改为普通属性 `action_menu`，赋值时把 `custom_minimum_size.x` 在 90 / 140 之间切换（等价于原 `_init(true)` 的取值），其余 `_init()` 设置的属性搬进场景；本场景的根脚本没有依赖注入方法，`setup` 记为空串（契约守卫只断言 `_init` 无参数）。原生坐标换算（`open_menu()` 的 `Transform2D` 与屏幕夹取）、键盘导航（`_key_input()`）、每帧自动收起（`_process()`）与全部公开方法/信号（`set_items`/`get_items`/`set_selected_id`/`get_selected_id`/`set_item_enabled`/`open_menu`/`close_menu`/`is_menu_open`）保持原样。

`res://scenes/ui/dropdown_item.tscn` + `src/ui/dropdown_item.gd`：下拉一行。根 `Button`（名 `DropdownItem`）承载静态外观（左对齐、`custom_minimum_size = (0,42)`、白底圆角 6 内边距 12 的 `normal`、`#f0f2f4` 圆角 6 内边距 12 的 `hover`、`#353c43` 的 `font_color`）与主题；`setup(item: Dictionary)` 注入行内容（`text` 取 `label`、`set_meta("id")`、`disabled`）。`unified_dropdown._build()` 的每一行改由该场景实例化后 `setup()` 再入树，分隔项仍是代码插入的 `HSeparator`；选中态前缀（`✓`）与四个字体颜色覆盖仍在 `open_menu()` 里逐次设置，行增量行为不变。

`res://scenes/ui/log_window.tscn`：根 `Window`（名 `LogWindow`）挂 `src/ui/log_window.gd` 与主题，`size (960,620)`、`min_size (660,400)`、默认不可见、`transient = false`；场景提供 `Panel`（`#111923`、圆角 0、内边距 14）、`Column`、`Filters`（`%RunsDropdown`（下拉场景实例，横向扩展填充）、`%LevelDropdown`、`%ModuleDropdown`）、`%Search`（占位「搜索时间、活动或错误码」）、`%Text`（`RichTextLabel`，可选中、纵向扩展、`#d6e5ee`、字号 14、等宽 `SystemFont`）、`Actions`（`%Follow`「跟随最新」、默认勾选、`#d6e5ee`；`%CopyButton`「复制显示记录」；`%ExportButton`「导出完整诊断 ZIP」）、`%Status`（`#ffd58a`、智能按词换行）与 `%Picker`（`FileDialog`，保存模式、文件系统访问、`*.zip` 过滤、原生对话框）。标题含发行版本号，仍由代码用 `src/release_info.gd` 拼装（不写进场景，避免版本号漂移）。`setup(logger)` 注入 `Log` 实例；`_ready()` 在依赖缺失时只做基类初始化并返回，`setup()` 在节点就绪（`is_node_ready()`）时由私有 `_initialize()` 完成一次性接线（三个筛选下拉的 `set_items` 与 `activated`、搜索框 `text_changed`、`_logger.entry_added` / `write_failed`、`close_requested`、文件选择回调）。`open()`、`_refresh()`、`_matches()`、`_append()` 与全部文案不变。

调用顺序固定为 `instantiate() → setup() → add_child()`：`application.gd` 的日志窗口与 `tests/test_log_window.gd` 的独立查看器都改用它；`src/ui/chat_view.gd`、`src/ui/model_window.gd`（`_selector`、`_copy`）、`src/ui/preferences_window.gd`（两个预设下拉）里由代码 `new()` 的五个下拉改为下拉场景实例（「更多」菜单额外把 `action_menu` 置为 `true`），`res://scenes/ui/account_view.tscn` 的 `%AccountMode` 由脚本节点改为下拉场景实例。

从哪个 interface 验证：`tests/test_ui_scenes.gd` 增加 `unified_dropdown.tscn`、`dropdown_item.tscn`、`log_window.tscn` 三条契约（根节点名与类型、根脚本、`_init` 无参数、无注入方法的场景 `setup` 记空串、关键节点 `unique_name_in_owner` 与 `%Name` 解析、搬入场景的属性与样式盒）；`tests/test_dropdown.gd`、`tests/capture_dropdown_ui.gd` 改用 `load("res://scenes/ui/unified_dropdown.tscn").instantiate()`，从原生弹层位置、长列表、键盘跳过禁用项、Enter/Esc、父窗口移动收起回归；`tests/test_log_window.gd` 改用日志窗口场景加 `setup(logger)`，从筛选条可读性、独立窗口唯一且可见、启动记录保留、实时追加、搜索过滤与归档不变回归。
### 消息气泡与虚拟列表场景化（0.1.2 第六片）

`res://scenes/ui/message_audio.tscn`：根 `VBoxContainer`（名 `MessageAudio`）挂 `src/ui/message_audio.gd` 与 `app_theme.tres`。场景提供 `%Row`（`HBoxContainer`，间距 6）、`%Play`（12 号字）、`%Wave`（`Control`，96×26、忽略鼠标）、`%Time`（11 号字、`#304553`）、`%Stop`（「停止」、12 号字）与 `%Error`（「语音未能保存」、12 号字）。波形自绘保留：原 `Waveform` 内部类改为独立脚本 `src/ui/message_waveform.gd`（`extends Control`，不新增 `class_name`）并挂在 `%Wave` 上，两个描线颜色改 `@export var played_color`（`#66ccff`）与 `@export var remaining_color`（`#9eb6c4`），取值在场景里赋值。根脚本删除建树代码与对 `src/preview/preview_style.gd` 的依赖，改 `@onready` + `%Name` 绑定；`signal action(value: String)` 与 `update_state(state: Dictionary) -> void` 的可用性、错误码文案（`CACHE_WRITE_FAILED` 为「语音未能保存」，否则「语音暂时无法重放」）、`idle`/`playing`/`paused` 三态按钮文案、`停止` 的可见性、`%.1f 秒` 与 `%.1f/%.1f 秒` 时间格式、波形与进度逐字不变。本场景没有依赖注入方法，契约守卫以 `setup` 记空串的方式只断言 `_init` 无参数。

`res://scenes/ui/message_bubble.tscn`：根 `HBoxContainer`（名 `MessageBubble`）挂 `src/preview/message_bubble.gd` 与主题，场景提供根的横向扩展填充与 `separation = 10`。节点树为 `%Avatar`（`TextureRect`，38×38、`EXPAND_IGNORE_SIZE`、`STRETCH_KEEP_ASPECT_CENTERED`、纵向 `SHRINK_BEGIN`、`round_avatar.gdshader` 的 `ShaderMaterial`）、`%Body`（`VBoxContainer`，间距 5）→ `%Bubble`（`PanelContainer`，白底圆角 12 内边距 13）→ `%Content`（`VBoxContainer`）内的 `%Text`（`RichTextLabel`，`fit_content`、`selection_enabled`、`scroll_active = false`、`AUTOWRAP_WORD_SMART`）、`%HistoryPicture`（`TextureRect`，`min_size.y = 135`、默认不可见）、`%ImageButton`（「加载图片…」、默认不可见）与可点的 `%Picture`（`TextureRect`，`min_size.y = 135`、手型光标、默认不可见），`%Body` 下另有 `%Caption`（11 号、右对齐、默认不可见），根下另有 `%System`（12 号、`#8a9ba4`、居中、横向扩展填充、默认不可见）。自己气泡的 `#dff3ff` 圆角 12 内边距 13 样式以 `@export var own_style: StyleBoxFlat` 在场景赋值，`configure()` 在自己气泡时覆盖 `%Bubble` 的 `panel`、否则移除覆盖，非自己的白底样式即场景默认值。头像纹理仍按角色在代码取 `res://assets/ui/user_icon.png` 与 `res://assets/ui/tianyi_icon.png`；自己气泡仍用 `move_child` 把 `%Avatar` 移到末尾，保持「头像在右」的既有布局。三个信号 `audio_action(action)`、`image_opened(texture)`、`image_action(action)` 与 `configure(message, image_texture = null)`、`update_message(message)`、`set_audio_state(state)`、`set_image_state(state)` 的公开面、状态文案表（等待历史同步／等待发送／已发送／发送中／发送失败／无法确认送达，` · 演示` 后缀与 `#b57373`／`#93a6af` 两色）、图片状态文案表（加载图片…／正在下载图片…／打开原图／图片加载失败 · 重试，` · 未能缓存` 后缀与 `[idle, loading]` 两态禁用）逐字不变。`system` 角色改为隐藏 `%Avatar` 与 `%Body`、显示 `%System`，不再新建节点；消息树里因此多出一个空文本的 `RichTextLabel`，但可见渲染路径与节点绑定关系不变。语音条改为实例化 `message_audio.tscn`（仍仅在 `state.available` 或错误码非空时创建，仍挂在 `%Body` 末尾并转发 `action`）。

`res://scenes/ui/virtual_message_list.tscn`：根 `ScrollContainer`（名 `VirtualMessageList`）挂 `src/ui/virtual_message_list.gd` 与主题；场景提供 `%Canvas`（`Control`，横向扩展填充）与根的 `SCROLL_MODE_DISABLED` 横向滚动。行的创建改为 `message_bubble.tscn`：`instantiate() → %Canvas.add_child() → configure(message)`。公开面不变：`visible_messages(ids)`、`interacted`、`audio_action(id, action)`、`image_opened(texture)`、`image_action(id, action)` 与 `set_messages`、`scroll_to_message`、`scroll_to_latest`、`get_visible_ids`、`get_reading_anchor`、`is_at_latest`、`set_audio_state`、`set_image_state`；`_layout`/`_render`/`_process` 的增量测高、锚点保持与二分定位、`_nodes` 行复用（语音状态更新不重建气泡）不变。本场景没有依赖注入方法，契约守卫以 `setup` 记空串的方式只断言 `_init` 无参数。

消费点：`src/ui/chat_view.gd` 的消息列表改 `preload("res://scenes/ui/virtual_message_list.tscn").instantiate()`；`src/preview/chat_preview.gd` 的离线样例行改 `preload("res://scenes/ui/message_bubble.tscn").instantiate()`。两者对列表/气泡的公开调用面与信号连接不变。

从哪个 interface 验证：`tests/test_ui_scenes.gd` 增加 `message_audio.tscn`、`message_bubble.tscn`、`virtual_message_list.tscn` 三条契约（场景存在性、根节点名与类型、根脚本、`_init` 无参数、无注入方法的场景 `setup` 记空串、关键节点 `unique_name_in_owner` 与 `%Name` 解析、从代码搬入场景的属性与样式盒、根带 `app_theme`）；`tests/test_virtual_history.gd` 改用 `load("res://scenes/ui/virtual_message_list.tscn").instantiate()`，从 1000 条消息的有界渲染节点（`RichTextLabel < 40`）、按 UUID 跳转与未知 UUID 不伪造跳转、前插与改宽后的精确阅读锚点、语音状态更新保留 RichTextLabel 选区、回到底部回归。
### chat_view 与 image_overlay 场景化（0.1.2 第七片）

`res://scenes/ui/chat_view.tscn`：根 `MarginContainer`（名 `ChatView`）挂 `src/ui/chat_view.gd` 与 `app_theme.tres`，依赖由 `setup(session)` 注入；删除带参 `_init`、`_draw()` 与全部建树代码，绑定改 `@onready` + `%Name`。根的四边边距为 0，原 `_draw()` 的整幅 `#f5f8fb` 填色改由根下的 `Background`（`ColorRect`、忽略鼠标）承担（`ColorRect` 放在根上而非内边距容器里，才能覆盖整个视图矩形）；原 22 像素四边内边距下移到 `%Margin`（`MarginContainer`，四边 22），其下是 `Column`（`VBoxContainer`，间距 12）。`Column` 的子节点顺序与改造前的 `add_child` 顺序逐项一致：`Heading`（`HBoxContainer`，间距 12）→ `HSeparator` → `HistoryRow` → `%Scroll` → `%Empty` → `%Latest` → `%Unread` → `HSeparator` → `AudioControls` → `%Input` → `Footer`。

`Heading` 内依次是 `Avatar`（`TextureRect`，42×42、`EXPAND_IGNORE_SIZE`、`STRETCH_KEEP_ASPECT_CENTERED`、纵向 `SHRINK_BEGIN`、`round_avatar.gdshader` 的 `ShaderMaterial`、`tianyi_icon.png`）、`Identity`（`VBoxContainer`，横向扩展填充）内的 `和天依聊聊`（22 号、`#344c59`）与 `%Status`（`AUTOWRAP_WORD_SMART`、13 号、`#607f8d`）、`%Dynamics`（「动态」）与 `%ChatMore`（`unified_dropdown.tscn` 的实例，即 `Button`）。`%ChatMore` 的节点名是外部契约：`tests/test_live_chat.gd`、`tests/test_voice_chat.gd`、`tests/test_application_window.gd`、`tests/test_application_drafts.gd`、`tests/capture_release_ui.gd` 都按这个名字定位，且必须唯一；它的 `action_menu`、`text`（「更多 ···」）与 `set_items`（`logs`/`cache`/`preferences`/`models`/`logout`）仍由脚本在依赖注入后设置，场景不写标题。`HistoryRow` 内是 `%HistoryStatus`（横向扩展填充、`AUTOWRAP_WORD_SMART`、12 号，无颜色覆盖）、`%HistoryRetry`（「重试历史」）与 `%HistorySkip`（「跳过本次」）。`AudioControls` 内是 `语音音量`（12 号、`#344c59`）、`%Volume`（`HSlider`，0–1、步长 .01、`custom_minimum_size.x = 110`、横向扩展填充、tooltip「回复语音自动播放；拖到最左侧静音」）与 `%StopVoice`（「停止语音」）。`%Input` 是带 `src/preview/composer_input.gd` 的 `TextEdit`（`想问些什么？`、`custom_minimum_size.y = 92`、`LINE_WRAPPING_BOUNDARY`）；`Footer` 内是 `Enter 发送 · Shift + Enter 换行`（11 号、`#94a5af`、横向扩展填充）与 `%Send`（「发送  ↑」、`custom_minimum_size.x = 96`、`PrimaryButton` 变体）。`%Empty`（「从一句问候开始」、居中）、`%Latest`（「回到最新 ↓」、默认不可见）、`%Unread`（「定位未读」、默认不可见）保持原属性。`%Scroll` 仍挂 `virtual_message_list.tscn`（横向滚动由该场景自身承担），纵向扩展填充。根下另有 `%ClearDialog`（`ConfirmationDialog`，标题「清理语音缓存」、正文「清理当前服务器、本账号保存的全部语音？\n聊天文字保留；已清理的语音将无法重放。」、「清理」/「取消」，默认不可见），它是视图子树内唯一的 `ConfirmationDialog`，测试按数量断言。

`res://scenes/preview/image_overlay.tscn`：根 `PanelContainer`（名 `ImageOverlay`）挂 `src/preview/image_overlay.gd` 与 `app_theme.tres`，依赖由 `setup(texture, pending)` 注入；`panel` 样式盒 `#f7fafc` 圆角 16 内边距 20（原 `Style.box`）。节点树为 `Column`（`VBoxContainer`，间距 12）内的 `Bar`（`HBoxContainer`）与 `%Title`（19 号、`#344c59`）加上 `%ZoomOut`（「－」）、`%ZoomIn`（「＋」）、`%Close`（`pending` 为真时「取消」、否则「关闭」，回调仍是 `queue_free`），`%Scroll`（`ScrollContainer`，纵向扩展填充）内的 `%Picture`（`TextureRect`，纹理由 `setup` 注入、`EXPAND_IGNORE_SIZE`、`STRETCH_KEEP_ASPECT_CENTERED`、双向扩展填充），以及 `%Feedback`（`Label`、`AUTOWRAP_WORD_SMART`）与 `%Confirm`（「发送图片 · 离线演示」）。`pending` 为假时原实现根本不创建这两个节点，场景改为常驻但 `visible = false`：可见外观等价，公开字段 `feedback`（改绑 `%Feedback`）仍可直接写文本，`pending` 为真时二者可见且 `%Confirm` 发 `confirmed(texture)`。标题文案（`待发送图片`／`图片预览`）、缩放区间 `clampf(_zoom * factor, 0.3, 4.0)`、`resized` 触发的重算、`ui_cancel` 的 `set_input_as_handled()` + `queue_free()` 与 `signal confirmed(texture)` 逐字保留；`_title`、缩放与 Esc 处理仍留在脚本里。

注入与调用顺序：两个场景都按 `instantiate() → setup(...) → add_child()` 使用，`setup()` 只保存依赖并在节点已就绪时调私有 `_initialize()`（`_initialized` 保护），`_ready()` 只在依赖已注入时初始化，因此无依赖实例化时场景节点树原样保留（契约守卫与场景检查的前提）。`chat_view` 的依赖为聊天会话（`_session` 为空时 `_ready()` 直接返回），`image_overlay` 的依赖为纹理（纹理为空时同样返回）。`chat_view` 仍留在脚本里的行为：输入框自动增高（`text_changed` 里按行数 `clampf(lines * 24 + 30, 92, 150)`）、状态与历史文案映射表、阅读位置/未读定位、`_refresh`/`_visible_audio`/`_to_latest`、浮层打开与重试分支、`get_window().focus_entered` 的阅读上报。公开面不变：`logout_requested`、`log_requested`、`settings_requested(kind)`、`set_dynamics_unread(count)`、`confirmed(texture)`、公开字段 `feedback` 与其余全部方法原样保留。浮层的父节点改为 `%Margin`：原实现把浮层 `add_child` 到带 22 像素边距的根上，`MarginContainer` 会把它压到内容区（比外框小 22 像素四边），挂到 `%Margin` 后几何与改造前逐像素一致，且仍是视图后代的 `Control`（`tests/test_history_media.gd` 与 `tests/capture_voice_ui.gd` 的可见控件边界检查因此不变）。

消费点：`src/application.gd` 的 `const ChatView` 改指 `res://scenes/ui/chat_view.tscn`，登录时 `instantiate() → setup(_chat)` 后再 `_split.add_child`（`custom_minimum_size.x = 440`、横向扩展填充与三个信号连接不变，`_split.move_child(_avatar, 0)` 保证视图仍是 `%Split` 的第 1 个子）；`src/ui/chat_view.gd` 的 `ImageOverlay` 与 `src/preview/chat_preview.gd` 的 `ImageOverlay` 常量改指 `res://scenes/preview/image_overlay.tscn`，两处都改 `instantiate() → setup(texture, pending) → add_child`。本片后 `chat_view.gd` 不再引用 `src/preview/preview_style.gd`（`Style` 常量随 `_draw()` 一起删除）；`chat_preview.gd` 自身的场景化与 `preview_style.gd` 的删除留待后续切片，`unified_dropdown.gd` 的 `Style.ACCENT` 也仍在。

从哪个 interface 验证：`tests/test_ui_scenes.gd` 增加 `chat_view.tscn`、`image_overlay.tscn` 两条契约（场景存在性、根节点名与类型、根脚本、`_init` 无参数、关键节点 `unique_name_in_owner` 与 `%Name` 解析、从代码搬入场景的属性与 `image_overlay` 的 `panel` 样式盒、根带 `app_theme`）；`tests/test_live_chat.gd`、`tests/test_voice_chat.gd`、`tests/test_history_media.gd`、`tests/capture_voice_ui.gd` 的场景门禁与实例化改指 `res://scenes/ui/chat_view.tscn` 并改用 `setup(session)`，从单输入框、`ChatMore` 菜单、清理缓存确认框、音量条、只读气泡文本、`打开原图`→`图片预览`、可见控件不出视口回归。

### 动态窗口与详情面板场景化（0.1.2 第八片）

`res://scenes/ui/dynamics_window.tscn`：根 `Window`（名 `DynamicsWindow`）挂 `src/ui/dynamics_window.gd` 与 `app_theme.tres`，原 `_init()` 的六个赋值全部搬进场景：`title`「天依的动态」、`size (1000,780)`、`min_size (900,640)`、默认不可见、`transient = false`、`force_native = true`。场景提供 `Panel`（`#f5f8fb`、圆角 0、内边距 16）→ `Column`（间距 12）→ `Toolbar`（`%Unread`（20 号字、横向扩展填充）、`%Publish`「发布动态」、`%Refresh`「刷新」、`%ReadAll`「全部已读」）、`%Notice`（12 号字、智能按词换行）、`%Split`（`HSplitContainer`，纵向扩展填充）与其两栏：`Left`（`VBoxContainer`，最小宽 270）内的 `%ListScroll`（`ScrollContainer`，禁用横向滚动、纵向扩展填充）→ `%List`（`VBoxContainer`，间距 5、横向扩展填充），`%More`（按钮）加在 `Left` 内、`%ListScroll` 之后，以及 `Right`（`PanelContainer`，最小宽 330、白底圆角 12 内边距 0）内的 `%Empty`（「选择一条动态」、水平与垂直居中）。`%Split` 的第 0 个子节点仍是左栏；分栏比例 `_ratio`（默认 .45，读写配置时夹取 .25–.7）与 `_resize_split()`/`_save_layout()` 仍留在脚本里。

`res://scenes/ui/dynamics_post_row.tscn` + `src/ui/dynamics_post_row.gd`：动态列表的一行，原 `_update()` 里就地 `Button.new()` 的建行代码改为场景实例。根 `Button`（名 `DynamicsPostRow`）承载 `custom_minimum_size = (0,102)` 与 `clip_contents = true`；`Content`（`HBoxContainer`，四边内缩 14/14/14/10）内是 `%Avatar`（`TextureRect`，38×38、`EXPAND_IGNORE_SIZE`、`STRETCH_KEEP_ASPECT_CENTERED`、纵向 `SHRINK_BEGIN`、`round_avatar.gdshader` 的 `ShaderMaterial`）与 `%Labels`（`VBoxContainer`，横向扩展填充）内的 `%Author`（14 号）、`%Meta`（11 号）、`%Excerpt`（14 号）三个 `#344c59` 标签，三者都带 `OVERRUN_TRIM_ELLIPSIS`。原 `_ignore_mouse(content)` 的递归忽略鼠标改由场景给这五个 `Control` 写 `mouse_filter = IGNORE`（`Button` 自身仍接收点击），`_ignore_mouse()` 随之删除。`setup(post, avatar_path)` 注入行内容：作者名、`TYPES.get(post.get("source_type",""),"动态")+" · "+post.created_at`、`post.content.replace("\n"," ")` 与头像纹理（`TYPES` 映射表随这段文本搬进行脚本）。行名 `"Post_"+post.id`、`pressed` 回调、按 index 的 `move_child()` 重排与选中态样式仍是窗口里的代码。
`res://scenes/ui/dynamic_comment_row.tscn` + `src/ui/dynamic_comment_row.gd`：详情里的一条评论，原 `update_comments()` 里就地 `VBoxContainer.new()` 的建行代码改为场景实例。根 `VBoxContainer`（名 `DynamicCommentRow`）承载 `Line`（`HBoxContainer`）与 `Sep`（`HSeparator`），`Line` 内是 `%Avatar`（`TextureRect`，34×34，其余同动态行头像）与 `Content`（`VBoxContainer`，横向扩展填充）内的 `%Author`（14 号、`#65717d`）、`%Body`（`RichTextLabel`，`fit_content`、`selection_enabled`、`scroll_active = false`）与 `Footer`（`HBoxContainer`）里的 `%Time`（12 号、`#94999f`）和 `%Reply`（`flat = true`、「回复」）。`setup(avatar_path, author, body, time_text, created_at, allow_comment, reply_action)` 注入行内容：`body` 仍是调用方拼好的「回复 %s：」前缀加正文，`created_at` 作为 `%Time` 的 `tooltip_text`，`allow_comment` 决定 `%Reply.disabled`（与原实现一致，只在建行时算一次，帖子可评论性变化不回溯已建的行），`reply_action` 是 `%Reply.pressed` 的回调。

`res://scenes/ui/dynamic_detail.tscn`：根 `ScrollContainer`（名 `DynamicDetail`）挂 `src/ui/dynamic_detail.gd` 与 `app_theme.tres`，原 `_init()` 的禁用横向滚动与双向扩展填充搬进场景；`Margin`（四边 18、横向扩展填充）→ `%Column`（`VBoxContainer`，间距 14、横向扩展填充）的子节点顺序与改造前的 `add_child` 顺序逐项一致：`Header`（`%Avatar`（42×42 圆形头像）加 `Identity` 里的 `%Author`（18 号、`#344c59`）与 `%CreatedAt`（12 号、`#94999f`））、`%Body`（`RichTextLabel`，`fit_content`、`selection_enabled`、`scroll_active = false`）、`%Notice`（「此动态不可评论。」）、`%CommentDraft`（`TextEdit`，占位「写下你的留言…」、最小高 90、边界换行、`#f5f7fa` 圆角 8 内边距 12 的 `normal`）、`%Send`（「发送评论」、`SIZE_SHRINK_END`、`PrimaryButton`）、`HSeparator`、`CommentTitle`（「评论 · 仅你与天依可见」、15 号、`#818991`）、`%Comments`（`VBoxContainer`，间距 16）、`%ReplyBox`（默认不可见，内含 `%ReplyLabel`、`%ReplyDraft`（占位「写下回复…」，其余同 `%CommentDraft`）、`%CancelReply`「取消回复对象」与 `%ReplySend`「发送回复」（`PrimaryButton`））、`%Status`（智能按词换行）与 `%LoadMore`。三个测试依赖的节点名 `CommentDraft`、`ReplyDraft`、`CancelReply` 由场景提供并带 `unique_name_in_owner`，详情实例本身仍是每帖一个、只隐藏不重建，`CommentDraft` 与作为详情根的 `ScrollContainer`（带 `update_comments()`，与列表侧不带该方法的 `ScrollContainer` 区分）因此保持同实例。

`dynamic_detail.gd` 仍留在脚本里的行为：评论分页与重试文案（「加载中…」/「加载更多评论」/「加载评论」/「重试评论」与「评论加载失败（%s），已保留现有内容。」）、`%ReplyBox` 的 reparent（`_place_reply()` 在整列与具体评论行之间搬移，取消回复对象后已输入文字降级为留言草稿）、评论行的增量创建与按 index 重排、`update_post()`/`is_dirty()`/`refresh_comments()`/`_submit()`/`_at_bottom()`；原 `_setup_input()` 写的两个输入框属性（占位、最小高 90、边界换行与 `normal` 样式盒）改由场景提供。`avatar_path(item)` 与 `relative_time(raw)` 仍是 `dynamic_detail.gd` 的静态方法（同 UI 模块共享的展示助手），窗口侧用 `const Detail = preload("res://src/ui/dynamic_detail.gd")` 调用 `avatar_path()`，详情视图另用 `const DetailScene` 常量。
`dynamics_window.gd` 删除带参 `_init(controller, layout_path)`，改 `setup(controller, layout_path = "user://dynamics-window.cfg")`；布局文件里的 `size` 与 `ratio` 读取从 `_init` 挪进 `_initialize()`，`_ready()` 在依赖缺失时只做基类初始化便返回，因此无依赖实例化时场景节点树原样保留（契约守卫的前提）。窗口仍留在脚本里的行为：`get_selected_id()`/`select_post()`/`is_dirty()`/`_update()`/`_select_style()`/`_refresh()`/`_open_publisher()`/`_at_bottom()`/`_resize_split()`/`_save_layout()` 与全部公开方法、信号；选中态样式（`#e3f5ff` 底、选中行加 `#66ccff` 左边框与宽 4）由 `Style.box` 改为就地构造 `StyleBoxFlat`，本片后 `dynamics_window.gd` 与 `dynamic_detail.gd` 都不再引用 `src/preview/preview_style.gd`。

消费点：`src/application.gd` 的 `kind == "dynamics"` 分支改为 `preload("res://scenes/ui/dynamics_window.tscn").instantiate()` 加 `setup(_dynamics, 布局目录/dynamics-window.cfg)` 再 `add_child`；`dynamics_window.gd` 的详情与动态行、`dynamic_detail.gd` 的评论行都按 `instantiate() → setup(...) → add_child()` 使用（行组件的 `setup()` 只保存注入内容，`_ready()` 在进入树时应用，因此 `@onready` 绑定可用）；发布窗口（`res://scenes/ui/publish_window.tscn`）的实例化与调用顺序不变。

从哪个 interface 验证：`tests/test_ui_scenes.gd` 增加 `dynamics_window.tscn`、`dynamic_detail.tscn`、`dynamics_post_row.tscn`、`dynamic_comment_row.tscn` 四条契约（场景存在性、根节点名与类型、根脚本、`_init` 无参数、关键节点 `unique_name_in_owner` 与 `%Name` 解析、从代码搬入场景的属性与样式盒、根带 `app_theme`）；`tests/test_dynamics_window.gd`、`tests/test_dynamics_detail.gd`、`tests/capture_dynamics_ui.gd` 的场景门禁与实例化改指 `res://scenes/ui/dynamics_window.tscn` 并改用 `setup(controller, layout_path)`，从发布按钮与失败草稿保留、只读动态禁用输入、每帖阅读位置保持、评论与动态分页、内联回复与取消回复保留文字、发布成功选中新动态并关闭发布窗口、默认 45:55 与拖动后跨窗口持久化回归。

### 头像面板与预览场景化（0.1.2 第九片）

`res://scenes/avatar/avatar_panel.tscn`：根 `Control`（名 `AvatarPanel`）挂 `src/avatar/avatar_panel.gd` 与 `app_theme.tres`。本视图没有 `setup()`：它对外只有两个公开字段 `avatar`（驱动实例）与 `framing`（构图实例），上层直接调 `avatar.*`，所以场景必须承载驱动节点本身。场景提供根的 `clip_contents = true` 与原来由 `_ready()` 建出的五个子节点（顺序即绘制顺序）：`Background`（`TextureRect`，`bg2.jpg`、`EXPAND_IGNORE_SIZE`、`STRETCH_KEEP_ASPECT_COVERED`、忽略鼠标、全屏锚点）、`Veil`（`ColorRect`，`Color(0.96,0.98,1.0,0.18)`、忽略鼠标、全屏锚点）、`%Driver`（挂 `src/avatar/avatar_driver.gd` 的 `Node2D`）、`%Error`（`Label`，`position = Vector2(20,80)`、`AUTOWRAP_WORD_SMART`、忽略鼠标）与 `%Reset`（`Button`，文案「重置位置」、tooltip「滚轮缩放 · 右键拖动」、左下锚点）。`Reset` 在场景里 `visible = false`，脚本在角色加载成功后才把它显示出来——原实现的加载失败路径在 `return` 之前不会建出这个按钮，常驻不可见节点保持同一可见行为。脚本删除建树代码，改 `@onready` + `%Name` 绑定，`avatar` 与 `_error_label` 分别改绑 `%Driver` 与 `%Error`；`%Reset` 的 `position = Vector2(18, size.y - 50)`（`_ready()` 与每次 `resized` 各一次）、`pressed` → `framing.reset()` + `_layout_avatar()` + `_save()`、滚轮缩放 1.1 与 1/1.1、右键按下/抬起拖动、`InputEventMouseMotion` 拖动、拖动结束与窗口失焦写盘、`gui_input` 的 `accept_event()`、`_process` 最小化停绘、`_layout_avatar()` 的错误 Label 宽度与三处错误文案（「角色加载失败，请检查资源是否完整。」「未能恢复角色位置，已使用默认构图。」「当前角色位置无法保存，重启后将恢复上次设置。」）逐字保留；`SETTINGS = "user://avatar_framing.cfg"`、`framing` 仍是 `Framing.new()`、`avatar_framing.gd` 的全部行为不动。

不节点化的例外（Live2D 驱动，与草稿窗口基类并列的第二处）：`GDCubismUserModel`、`GDCubismEffectBreath` 与 `GDCubismEffectCustom` 仍由 `src/avatar/avatar_driver.gd` 在运行时 `ClassDB.instantiate` 后 `add_child`——模型路径在运行时才从 `assets/live2d/character.json` 解析，不能写进场景；场景承载的是挂该脚本的 `%Driver` 节点本身，`load_character` / `load_avatar` / `apply_expression` / `play_motion` / `set_mouth_openness` / `get_status` 六个入口的语义不变。

`res://scenes/avatar/avatar_preview.tscn`：`src/avatar/avatar_preview.gd` 继续 `extends "res://src/avatar/avatar_panel.gd"`，场景是 `avatar_panel.tscn` 的场景继承（根 `instance` + `script` 覆盖，根名 `AvatarPreview`），节点树、`clip_contents` 与主题全部由基场景提供。冒烟与截图行为逐字不变：`super._ready()` 之后 `avatar.get_status().loaded` 为假就 `push_error("Avatar load failed")` 并 `quit(1)`；`--capture=<路径>` 仍等 2 秒、`await RenderingServer.frame_post_draw` 后写 PNG，按保存结果 `quit(0/1)`。旧的 `res://scenes/avatar_preview.tscn`（只挂脚本、节点树由代码建）随之删除，`client_godot/README.md` 的路径引用改指新位置。
