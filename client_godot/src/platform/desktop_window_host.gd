extends "res://src/platform/window_host.gd"
...
func attach(window: Window, login: bool) -> void:
	...
	_window.close_requested.connect(save)
...
func open_window() -> void:
	if _window.mode == Window.MODE_MINIMIZED:
		...
		if _window != _window.get_tree().root: _window.hide()
...
func detach() -> void:
	save()
	if is_instance_valid(_window) and _window.close_requested.is_connected(save):
		_window.close_requested.disconnect(save)
	_window = null
