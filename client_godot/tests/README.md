...
当前有58个`test_*.gd`：55个由四组headless检查编排，3个仅用于图形/原生窗口验收。另有4个`capture_*.gd`视觉入口。目录表示模块职责，GPU能力由运行方式区分；未列入默认组不代表过时。
...
| 分组 | 当前内容 |
| --- | --- |
| check | 导入、启动、离线样板及31个Godot测试，共34项；无需Python，包含可靠队列与凭据基础检查 |
