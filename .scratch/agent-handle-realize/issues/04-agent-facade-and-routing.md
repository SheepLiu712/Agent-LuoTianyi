...
**What to build:** 让 `AgentRuntime.get_agent(character_id)` 返回稳定的 Agent façade；新 façade 对业务调用方只提供 `handle_stimulus` 与 `realize_action_plan`，统一做契约校验、角色绑定、错误转换、取消入口和 Handler 路由，但暂不迁移现有生产调用方。
...
- `handle_stimulus` 根据 StimulusKind + InteractionKind 唯一路由 Stimulus Handler；`realize_action_plan` 根据 ActionKind 唯一路由 Action Handler。
